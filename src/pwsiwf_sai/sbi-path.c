...
int pwsiwf_sai_sbi_open(void) 
{
    ...
    ogs_info("pwsiwf_sai_sbi_open: Starting %d SBI servers", count);
    if (ogs_sbi_server_start_all(pwsiwf_sai_sbi_server_callback) != OGS_OK) {
        ogs_error("Failed to start PWS-IWF_SAI SBI servers");
        return OGS_ERROR;
    }
    
    // Start SBI client for AMF communication
    if (pwsiwf_sai_sbi_client_open() != OGS_OK) {
        ogs_error("Failed to start PWS-IWF_SAI SBI client");
        return OGS_ERROR;
    }
...
{ 
    ...
    ogs_info("PWS-IWF_SAI SBI server callback received: %s %s", 
    ...
    CASE(OGS_SBI_SERVICE_NAME_NPWS_CALLBACK)
        ...
        CASE(OGS_SBI_RESOURCE_NAME_N1_N2_FAILURE_NOTIFY)
            ...
            CASE(OGS_SBI_HTTP_METHOD_POST)
                rv = pwsiwf_sai_nonuen2_comm_handle_nonuen2_message_transfer_failure_notify(
                        stream, &message);
                break;
            DEFAULT
...
{ 
    ogs_info("PWS-IWF_SAI SBI client opened");
    return OGS_OK; 
}

void pwsiwf_sai_sbi_client_close(void) 
{
    ogs_info("PWS-IWF_SAI SBI client closed");
}
...
{ 
    ...
    if (status != OGS_OK) {
        ogs_log_message(
                ...
                "pwsiwf_sai_sbi_client_callback() failed [%d]", status);
        return OGS_ERROR;
    }
        ...
        pwsiwf_sai_warning_t *warning, pwsiwf_sai_nonuen2_message_transfer_param_t *param)
{
    ...
    ogs_assert(warning);
    ogs_assert(param);
    ogs_assert(pwsiwf_sai_self()->amf_sbi);
    
    // Build the Non-UE N2 message transfer request
    request = pwsiwf_sai_nonuen2_comm_build_nonuen2_message_transfer(warning, param);
    if (!request) {
    ...
    rc = ogs_sbi_send_request_to_client(
            pwsiwf_sai_self()->amf_sbi, 
            pwsiwf_sai_sbi_client_callback, 
            request, 
            warning);
