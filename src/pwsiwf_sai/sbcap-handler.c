        ...
        switch(ie->id) {
            ...
            case SBCAP_ProtocolIE_ID_id_Warning_Message_Content:
                ...
                if (warning_message_content->size > 0 && warning_message_content->size <= 960) {
                    ...
                    // Print warning message content in hex format
                    char hex_content[2048] = {0};
                    int j;
                    for (j = 0; j < warning_message_content->size && j < sizeof(hex_content)/2-1; j++) {
                        sprintf(&hex_content[j*2], "%02X", (uint8_t)warning_message_content->buf[j]);
                    }
                    ogs_info("warning_message_content (hex): %s", hex_content);
                } else {
        ...
        pwsiwf_sai_connection_t *connection, ogs_sbcap_message_t *message)
{
    ...
    // Build and send Non-UE N2 Stop Warning to AMF
    pwsiwf_sai_nonuen2_message_transfer_param_t param;
    memset(&param, 0, sizeof(param));
    param.state = PWSIWF_SAI_WARNING_MESSAGE_STOP;
    param.n2smbuf = ngap_build_warning_message_stop_request_transfer(warning);
    if (!param.n2smbuf) {
        ogs_error("Failed to build NGAP stop warning request transfer");
        pwsiwf_sai_warning_remove(warning);
        return;
    }

    if (pwsiwf_sai_nonuen2_comm_send_nonuen2_message_transfer(warning, &param) != OGS_OK) {
        ogs_error("Failed to send Non-UE N2 stop warning to AMF");
        pwsiwf_sai_warning_remove(warning);
        return;
    }

    ogs_info("Forwarded Stop-Warning-Request to AMF");
