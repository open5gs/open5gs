    ...
    case SMF_NETWORK_REQUESTED_QOS_FLOW_MODIFICATION:
        ...
        if (recvmsg->res_status == OGS_SBI_HTTP_STATUS_OK) {
            if (N1N2MessageTransferRspData->cause ==
                ...
                /* The H-SMF stream was stored when the N1N2 transaction
                 * was created. NAS completion may already have used it. */
            } else {
            ...
            if (N1N2MessageTransferRspData->cause ==
                ...
                /* Keep the H-SMF stream saved at transaction creation.
                 * A late paging response must not restore a consumed ID. */
            } else {
                ogs_error("Not implemented [cause:%d]",
                        N1N2MessageTransferRspData->cause);
                ogs_assert_if_reached();
