...
static smf_redis_context_t self;
static ogs_timer_mgr_t *timer_mgr = NULL;


static void smf_redis_reconnect_timer_expire(void *data);
...
int smf_redis_init(void)
{
    ...
    /* Initialize Redis context */
    memset(&self, 0, sizeof(smf_redis_context_t));
    ...
    self.t_reconnect = ogs_timer_add(ogs_app()->timer_mgr,
        smf_redis_reconnect_timer_expire, NULL);
    if (!self.t_reconnect) {
    ...
    return smf_redis_connect();
}

void smf_redis_final(void)
{
    smf_redis_disconnect();
