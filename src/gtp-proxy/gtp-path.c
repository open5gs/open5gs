...
int sgwc_gtp_open(void)
{
    ...
    ogs_list_for_each(&ogs_gtp_self()->gtpc_list, node) {
        ...
        // Storing the last socket as a back-end interface
        self->gtpc_backend_sock = sock;
        self->gtpc_backend_addr = &sock->local_addr;
    }
    ogs_list_for_each(&ogs_gtp_self()->gtpc_list6, node) {
        ...
        // Storing the last socket as a back-end interface
        self->gtpc_backend_sock6 = sock;
        self->gtpc_backend_addr6 = &sock->local_addr;
    }

    if (self->gtpc_backend_addr) {
        ogs_info("GTP-C IPv4 back-end interface: %s:%d",
                OGS_ADDR(self->gtpc_backend_addr, buf), OGS_PORT(self->gtpc_backend_addr));
    }

    if (self->gtpc_backend_addr6) {
        ogs_info("GTP-C IPv6 back-end interface: %s:%d",
                OGS_ADDR(self->gtpc_backend_addr6, buf), OGS_PORT(self->gtpc_backend_addr6));
    }
