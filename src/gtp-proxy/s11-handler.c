...
// Re-encode a modified GTPv2 message
static ogs_pkbuf_t* ogs_gtp_reencode(ogs_pkbuf_t* gtpbuf, ogs_gtp2_message_t* message) {
    ogs_pkbuf_t* pkbuf = ogs_gtp2_build_msg(message);
    ogs_assert(pkbuf != NULL);

    // XXX ogs_gtp2_build_msg DOES NOT generate a GTP header
    // Copying the header from the original message, imitating what ogs_gtp_xact_update_tx does
    int gtp_hlen = 0;
    if (message->h.type > OGS_GTP2_VERSION_NOT_SUPPORTED_INDICATION_TYPE) {
        gtp_hlen = OGS_GTPV2C_HEADER_LEN;
    } else {
        gtp_hlen = OGS_GTPV2C_HEADER_LEN - OGS_GTP2_TEID_LEN;
    }
    ogs_pkbuf_push(pkbuf, gtp_hlen);
    memcpy(pkbuf->data, gtpbuf->data, gtp_hlen);
    return pkbuf;
}
        ...
        ogs_pkbuf_t *gtpbuf, ogs_gtp2_message_t *message)
{
    ...
    // Sergii: We construct pgw_s5_s8_address_for_control_plane_or_pmip below, after a local session record is generated
    ...
    // Sergii: Constructing pgw_s5_s8_address_for_control_plane_or_pmip to make S5 request look like S11
    // Inspiration:
    // - src/mme/mme-s11-build.c
    // - tests/unit/gtp-message-test.c
    ogs_gtp2_f_teid_t pgw_s5c_teid_hardcoded = {};
    pgw_s5c_teid_hardcoded.interface_type = OGS_GTP2_F_TEID_S5_S8_PGW_GTP_C;
    pgw_s5c_teid_hardcoded.teid = htobe32(sess->pgw_s5c_teid);
    pgw_s5c_teid_hardcoded.ipv4 = 1;
    pgw_s5c_teid_hardcoded.addr = sgwc_self()->smf_addr.sin.sin_addr.s_addr;

    req->pgw_s5_s8_address_for_control_plane_or_pmip.presence = 1;
    req->pgw_s5_s8_address_for_control_plane_or_pmip.data = &pgw_s5c_teid_hardcoded;
    req->pgw_s5_s8_address_for_control_plane_or_pmip.len = OGS_GTP2_F_TEID_IPV4_LEN;
    ...
    for (i = 0; i < OGS_BEARER_PER_UE; i++) {
        ...
        if (req->bearer_contexts_to_be_created[i].s1_u_enodeb_f_teid.presence) {
            enb_s1u_teid = req->bearer_contexts_to_be_created[i].s1_u_enodeb_f_teid.data;
        } else if (req->bearer_contexts_to_be_created[i].s5_s8_u_sgw_f_teid.presence) {
            enb_s1u_teid = req->bearer_contexts_to_be_created[i].s5_s8_u_sgw_f_teid.data;
        }
    ...
    // Sergii: Re-encode the modified GTPv2 message with CreateSession request.
    // PFCP transactions carry a copy of GTP-C message that triggered them.
    // TODO Get rid of this: we probably can just use the SMF address from the config in sxa-handler
    ogs_pkbuf_t* updated_gtpbuf = ogs_gtp_reencode(gtpbuf, message);

    ogs_assert(OGS_OK ==
        sgwc_pfcp_send_session_establishment_request(
            sess, s11_xact, updated_gtpbuf, 0));
    ogs_pkbuf_free(updated_gtpbuf);
}
