        ...
        ogs_pfcp_session_establishment_response_t *pfcp_rsp)
{
    ...
    rv = ogs_gtp2_sockaddr_to_f_teid(
            sgwc_self()->gtpc_backend_addr, sgwc_self()->gtpc_backend_addr6,
    ...
    if (!pgw) {
        ...
        rv = ogs_gtp_connect(
                sgwc_self()->gtpc_backend_sock, sgwc_self()->gtpc_backend_sock6, pgw);
        ogs_assert(rv == OGS_OK);
    }
    /* Setup GTP Node */
        ...
        ogs_pfcp_session_modification_response_t *pfcp_rsp)
{
    ...
    if (flags & OGS_PFCP_MODIFY_REMOVE) {
        ...
        if (flags & OGS_PFCP_MODIFY_UL_ONLY) {
            ...
            // Sergii: Set the GTP-C F-TEID to ourselves
            memset(&pgw_s5_teid, 0, sizeof(ogs_gtp2_f_teid_t));
            pgw_s5_teid.interface_type = OGS_GTP2_F_TEID_S5_S8_PGW_GTP_C;
            pgw_s5_teid.teid = htobe32(sgwc_ue->sgw_s11_teid);
            rv = ogs_gtp2_sockaddr_to_f_teid(
                    ogs_gtp_self()->gtpc_addr, ogs_gtp_self()->gtpc_addr6,
                    &pgw_s5_teid, &len);
            ogs_assert(rv == OGS_OK);
            gtp_rsp->pgw_s5_s8__s2a_s2b_f_teid_for_pmip_based_interface_or_for_gtp_based_control_plane_interface.presence = 1;
            gtp_rsp->pgw_s5_s8__s2a_s2b_f_teid_for_pmip_based_interface_or_for_gtp_based_control_plane_interface.data = &pgw_s5_teid;
            gtp_rsp->pgw_s5_s8__s2a_s2b_f_teid_for_pmip_based_interface_or_for_gtp_based_control_plane_interface.len = len;
            ...
            ogs_list_for_each_entry(
                ...
                // Sergii: Set the GTP-U F-TEID to ourselves
                memset(&pgw_s5u_teid[i], 0, sizeof(ogs_gtp2_f_teid_t));
                pgw_s5u_teid[i].interface_type = OGS_GTP2_F_TEID_S5_S8_PGW_GTP_U;
                pgw_s5u_teid[i].teid = htobe32(ul_tunnel->local_teid);
                ogs_assert(ul_tunnel->local_addr || ul_tunnel->local_addr6);
                rv = ogs_gtp2_sockaddr_to_f_teid(
                    ul_tunnel->local_addr, ul_tunnel->local_addr6,
                    &pgw_s5u_teid[i], &len);
                ogs_assert(rv == OGS_OK);
                gtp_rsp->bearer_contexts_created[i].s5_s8_u_sgw_f_teid.
                    presence = 1;
                gtp_rsp->bearer_contexts_created[i].s5_s8_u_sgw_f_teid.
                    data = &pgw_s5u_teid[i];
                gtp_rsp->bearer_contexts_created[i].s5_s8_u_sgw_f_teid.
