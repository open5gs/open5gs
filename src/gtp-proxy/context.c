...
void sgwc_context_init(void)
{
    ...
    ogs_log_install_domain(&__gtp_proxy_log_domain, "sgwc", ogs_core()->log.level);
...
static int sgwc_context_validation(void)
{
    if (ogs_list_empty(&ogs_gtp_self()->gtpc_list) &&
        ...
        ogs_error("No gtp-proxy.gtpc in '%s'", ogs_app()->file);
        return OGS_ERROR;
    }
    if (sgwc_self()->smf_addr.ogs_sa_family == 0) {
        ogs_error("No gtp-proxy.smf_address in '%s'", ogs_app()->file);
        return OGS_ERROR;
    }
    return OGS_OK;
}
        ...
        if (!strcmp(root_key, "gtp-proxy")) {
            ...
            while (ogs_yaml_iter_next(&sgwc_iter)) {
                ...
                if (!strcmp(sgwc_key, "gtpc")) {
                    ...
                    /* handle config in pfcp library */
                } else if (!strcmp(sgwc_key, "smf_address")){
                    ogs_assert(ogs_inet_pton(AF_INET, ogs_yaml_iter_value(&sgwc_iter), &sgwc_self()->smf_addr) == OGS_OK);
                } else
