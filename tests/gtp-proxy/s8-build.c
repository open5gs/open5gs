...
ogs_pkbuf_t *test_s8_build_create_session_request(
        uint8_t type, test_sess_t *sess, bool handover_ind)
{
    ...
    ogs_gtp2_f_teid_t test_s8_c_teid, test_s8_u_teid[OGS_BEARER_PER_UE];
    int len, test_s8_u_len;
    ogs_paa_t paa;
    ...
    memset(&test_s8_c_teid, 0, sizeof(ogs_gtp2_f_teid_t));
    test_s8_c_teid.interface_type = OGS_GTP2_F_TEID_S5_S8_SGW_GTP_C;
    test_s8_c_teid.teid = htobe32(sess->epdg_s2b_c_teid);
    ogs_assert(sess->gnode->sock);
    rv = ogs_gtp2_sockaddr_to_f_teid(
            &sess->gnode->sock->local_addr, NULL, &test_s8_c_teid, &len);
    ogs_assert(rv == OGS_OK);
    req->sender_f_teid_for_control_plane.presence = 1;
    req->sender_f_teid_for_control_plane.data = &test_s8_c_teid;
    ...
    ogs_list_for_each(&sess->bearer_list, bearer) {
        ...
        memset(&test_s8_u_teid[i], 0, sizeof(ogs_gtp2_f_teid_t));
        test_s8_u_teid[i].teid = htobe32(bearer->enb_s1u_teid);
        test_s8_u_teid[i].interface_type = OGS_GTP2_F_TEID_S5_S8_SGW_GTP_U;
        ogs_assert(sess->gnode->sock);
        rv = ogs_gtp2_sockaddr_to_f_teid(
                &sess->gnode->sock->local_addr, NULL,
                &test_s8_u_teid[i], &test_s8_u_len);

        req->bearer_contexts_to_be_created[i].s5_s8_u_sgw_f_teid.presence = 1;
...
ogs_pkbuf_t *test_s8_build_delete_session_request(
...
ogs_pkbuf_t *test_s8_build_create_bearer_response(
        uint8_t type, test_bearer_t *bearer)
{
    ...
    ogs_gtp2_f_teid_t epdg_s8_u_teid, smf_s8_u_teid;
    int len;
    ...
    memset(&epdg_s8_u_teid, 0, sizeof(ogs_gtp2_f_teid_t));
    epdg_s8_u_teid.interface_type = OGS_GTP2_F_TEID_S2B_U_EPDG_GTP_U;
    epdg_s8_u_teid.teid = htobe32(bearer->enb_s1u_teid);
    ogs_assert(sess->gnode->sock);
    rv = ogs_gtp2_sockaddr_to_f_teid(
            &sess->gnode->sock->local_addr, NULL, &epdg_s8_u_teid, &len);
    if (rv != OGS_OK) {
        ogs_error("ogs_gtp2_sockaddr_to_f_teid() failed");
        return NULL;
    }
    rsp->bearer_contexts.s2b_u_epdg_f_teid_8.presence = 1;
    rsp->bearer_contexts.s2b_u_epdg_f_teid_8.data = &epdg_s8_u_teid;
    rsp->bearer_contexts.s2b_u_epdg_f_teid_8.len = len;

    /* Data Plane(UL) : SMF-S2B-U */
    memset(&smf_s8_u_teid, 0, sizeof(ogs_gtp2_f_teid_t));
    smf_s8_u_teid.interface_type = OGS_GTP2_F_TEID_S2B_U_PGW_GTP_U;
    smf_s8_u_teid.teid = htobe32(bearer->sgw_s1u_teid);
    rv = ogs_gtp2_ip_to_f_teid(&bearer->sgw_s1u_ip, &smf_s8_u_teid, &len);
    if (rv != OGS_OK) {
        ogs_error("ogs_gtp2_ip_to_f_teid() failed");
        return NULL;
    }
    rsp->bearer_contexts.s2b_u_pgw_f_teid.presence = 1;
    rsp->bearer_contexts.s2b_u_pgw_f_teid.data = &smf_s8_u_teid;
...
ogs_pkbuf_t *test_s8_build_delete_bearer_response(
        uint8_t type, test_bearer_t *bearer)
{
    int rv;
    ogs_session_t *session = NULL;
