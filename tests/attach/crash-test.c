...
#if 0
static void test6_issues4357_func(abts_case *tc, void *data)
{
    int rv;
    ogs_socknode_t *s1ap;
    ogs_socknode_t *gtpu;
    ...
    test_ue = test_ue_add_by_suci(&mobile_identity_suci, "0000000010");
    ogs_assert(test_ue);
    ...
    /* Send Attach Request - INVALID IMSI */
    sess->pti = 1;
    test_ue->mobile_identity_imsi.odd_even = 0;
    ...
    memset(&test_ue->attach_request_param,
            0, sizeof(test_ue->attach_request_param));
    emmbuf = testemm_build_attach_request(test_ue, esmbuf, true, true);
    ABTS_PTR_NOTNULL(tc, emmbuf);
    ...
    /* Send Attach Request - Valid IMSI */
    sess->pti = 1;
    test_ue->mobile_identity_imsi.odd_even = 1;
...
#if 0 /* Commenting to suppress error messages */
    abts_run_test(suite, test6_issues4357_func, NULL);
#endif
