...
static void test1_func(abts_case *tc, void *data)
{
    ...
    ogs_socknode_t *ngap;
    ogs_socknode_t *gtpu;
    ogs_socknode_t *tngap;
    ogs_socknode_t *tgtpu;
    ogs_pkbuf_t *gmmbuf;
    ogs_pkbuf_t *gsmbuf;
    ogs_pkbuf_t *nasbuf;
    ogs_pkbuf_t *sendbuf;
    ...
    /* target gNB connects to AMF */
    tngap = testngap_client(AF_INET);
    ABTS_PTR_NOTNULL(tc, tngap);

    /* target gNB connects to UPF */
    tgtpu = test_gtpu_server(2, AF_INET);
    ABTS_PTR_NOTNULL(tc, tgtpu);
    ...
    rv = test_gtpu_send_ping(tgtpu, qos_flow, TEST_PING_IPV4);
    ABTS_INT_EQUAL(tc, OGS_OK, rv);
    
    /* Receive GTP-U ICMP Packet */
    recvbuf = testgnb_gtpu_read(tgtpu);
    ABTS_PTR_NOTNULL(tc, recvbuf);
    ogs_pkbuf_free(recvbuf);
    ...
    testgnb_gtpu_close(tgtpu);

    /* gNB disonncect from AMF */
    testgnb_ngap_close(ngap);
    testgnb_ngap_close(tngap);

    /* Clear Test UE Context */
    test_ue_remove(test_ue);
}

abts_suite *test_amf_xn(abts_suite *suite)
{
