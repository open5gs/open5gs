...
int tlv_build_msg(ogs_pkbuf_t **pkbuf, tlv_desc_t *desc, void *msg, int mode)
{
    ...
    ogs_assert(desc->ctype == TLV_MESSAGE);
    ...
    *pkbuf = ogs_pkbuf_alloc(NULL, TLV_MAX_HEADROOM+length);
    ogs_assert(*pkbuf);
...
static tlv_desc_t* tlv_find_desc(uint8_t *desc_index,
        uint32_t *tlv_offset, tlv_desc_t *parent_desc, ogs_tlv_t *tlv)
{
...
static int tlv_parse_leaf(void *msg, tlv_desc_t *desc, ogs_tlv_t *tlv)
{
    ogs_assert(msg);
    ogs_assert(desc);
    ogs_assert(tlv);
...
static int tlv_parse_compound(void *msg, tlv_desc_t *parent_desc, 
