        ...
        uint8_t type, ogs_pfcp_user_plane_report_t *report)
{
    ...
    if(report->type.usage_report) {
        ...
        req->usage_report.usage_report_trigger.u24 = (report->usage_report.rep_triggers.reptri_5 << 16) 
        ...
        if (report->usage_report.vol_measurement.flags) {
            ...
            ogs_pfcp_build_volume_measurement(
                    &req->usage_report.volume_measurement, &report->usage_report.vol_measurement,
                    &usage_report_buf.vol_meas, sizeof(usage_report_buf.vol_meas));
        }

        if (report->usage_report.dur_measurement) {
            req->usage_report.duration_measurement.presence = 1;
            req->usage_report.duration_measurement.u32 = report->usage_report.dur_measurement;
        }
        
        if(report->usage_report.time_of_first_packet) {
            req->usage_report.time_of_first_packet.presence = 1;
            req->usage_report.time_of_first_packet.u32 = report->usage_report.time_of_first_packet;
        }

        if(report->usage_report.time_of_last_packet) {
            req->usage_report.time_of_last_packet.presence = 1;
            req->usage_report.time_of_last_packet.u32 = report->usage_report.time_of_last_packet;
        }
