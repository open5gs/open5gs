    ...
    ogs_pfcp_tlv_create_far_t *message, int i, ogs_pfcp_far_t *far)
{
    ...
    if (far->apply_action & OGS_PFCP_APPLY_ACTION_FORW) {
	    ...
	    message->forwarding_parameters.network_instance.len = ogs_fqdn_build(
			 farbuf[i].dnn, far->dnn, strlen(far->dnn));
	    message->forwarding_parameters.network_instance.data = farbuf[i].dnn;
        }
        ...
        ogs_pfcp_tlv_update_far_t *message, int i, ogs_pfcp_far_t *far)
{
    ...
    if (far->outer_header_creation_len || far->smreq_flags.value) {
        ...
        if (far->outer_header_creation_len) {
	       ...
	       message->update_forwarding_parameters.network_instance.len = ogs_fqdn_build(
			 farbuf[i].dnn, far->dnn, strlen(far->dnn));
	       message->update_forwarding_parameters.network_instance.data = farbuf[i].dnn;
            }
