...
cJSON *OpenAPI_ee_profile_data_convertToJSON(OpenAPI_ee_profile_data_t *ee_profile_data)
{
    ...
    if (ee_profile_data->restricted_event_types != OpenAPI_event_type_NULL) {
...
OpenAPI_ee_profile_data_t *OpenAPI_ee_profile_data_parseFromJSON(cJSON *ee_profile_dataJSON)
{
    ...
    if (restricted_event_types) {
        ...
        if (restricted_event_typesList->count == 0) {
            ogs_error("OpenAPI_ee_profile_data_parseFromJSON() failed: Expected restricted_event_typesList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
