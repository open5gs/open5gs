...
)
{
    ...
    wireline_area_1_local_var->comb_gci_and_hfc_n_ids = comb_gci_and_hfc_n_ids;
...
void OpenAPI_wireline_area_1_free(OpenAPI_wireline_area_1_t *wireline_area_1)
{
    ...
    if (wireline_area_1->comb_gci_and_hfc_n_ids) {
        OpenAPI_list_for_each(wireline_area_1->comb_gci_and_hfc_n_ids, node) {
            OpenAPI_comb_gci_and_hfc_n_ids_1_free(node->data);
        }
        OpenAPI_list_free(wireline_area_1->comb_gci_and_hfc_n_ids);
        wireline_area_1->comb_gci_and_hfc_n_ids = NULL;
    }
...
cJSON *OpenAPI_wireline_area_1_convertToJSON(OpenAPI_wireline_area_1_t *wireline_area_1)
{
    ...
    if (wireline_area_1->global_line_ids) {
    cJSON *global_line_idsList = cJSON_AddArrayToObject(item, "globalLineIds");
    if (global_line_idsList == NULL) {
        ogs_error("OpenAPI_wireline_area_1_convertToJSON() failed [global_line_ids]");
        goto end;
    }
    OpenAPI_list_for_each(wireline_area_1->global_line_ids, node) {
        if (cJSON_AddStringToObject(global_line_idsList, "", (char*)node->data) == NULL) {
            ogs_error("OpenAPI_wireline_area_1_convertToJSON() failed [global_line_ids]");
            goto end;
        }
    }
    ...
    OpenAPI_list_for_each(wireline_area_1->hfc_n_ids, node) {
        if (cJSON_AddStringToObject(hfc_n_idsList, "", (char*)node->data) == NULL) {
            ogs_error("OpenAPI_wireline_area_1_convertToJSON() failed [hfc_n_ids]");
            goto end;
        }
    }
    ...
    if (comb_gci_and_hfc_n_idsList == NULL) {
        ogs_error("OpenAPI_wireline_area_1_convertToJSON() failed [comb_gci_and_hfc_n_ids]");
        goto end;
    }
    OpenAPI_list_for_each(wireline_area_1->comb_gci_and_hfc_n_ids, node) {
        cJSON *itemLocal = OpenAPI_comb_gci_and_hfc_n_ids_1_convertToJSON(node->data);
        if (itemLocal == NULL) {
            ogs_error("OpenAPI_wireline_area_1_convertToJSON() failed [comb_gci_and_hfc_n_ids]");
            goto end;
        }
...
OpenAPI_wireline_area_1_t *OpenAPI_wireline_area_1_parseFromJSON(cJSON *wireline_area_1JSON)
{
    ...
    comb_gci_and_hfc_n_ids = cJSON_GetObjectItemCaseSensitive(wireline_area_1JSON, "combGciAndHfcNIds");
    if (comb_gci_and_hfc_n_ids) {
        cJSON *comb_gci_and_hfc_n_ids_local = NULL;
        if (!cJSON_IsArray(comb_gci_and_hfc_n_ids)) {
            ogs_error("OpenAPI_wireline_area_1_parseFromJSON() failed [comb_gci_and_hfc_n_ids]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(comb_gci_and_hfc_n_ids_local, comb_gci_and_hfc_n_ids) {
            ...
            OpenAPI_comb_gci_and_hfc_n_ids_1_t *comb_gci_and_hfc_n_idsItem = OpenAPI_comb_gci_and_hfc_n_ids_1_parseFromJSON(comb_gci_and_hfc_n_ids_local);
