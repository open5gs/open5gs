...
{
    OpenAPI_wlan_measurement_t *wlan_measurement_local_var = ogs_malloc(sizeof(OpenAPI_wlan_measurement_t));
    ogs_assert(wlan_measurement_local_var);

    wlan_measurement_local_var->measurement_name_list = measurement_name_list;
    wlan_measurement_local_var->wlan_rssi = wlan_rssi;
    wlan_measurement_local_var->wlan_rtt = wlan_rtt;

    return wlan_measurement_local_var;
}
...
void OpenAPI_wlan_measurement_free(OpenAPI_wlan_measurement_t *wlan_measurement)
{
    ...
    if (NULL == wlan_measurement) {
        return;
    }
    if (wlan_measurement->measurement_name_list) {
        OpenAPI_list_for_each(wlan_measurement->measurement_name_list, node) {
            OpenAPI_measurement_name_free(node->data);
        }
        OpenAPI_list_free(wlan_measurement->measurement_name_list);
        wlan_measurement->measurement_name_list = NULL;
    }
    ogs_free(wlan_measurement);
}
...
cJSON *OpenAPI_wlan_measurement_convertToJSON(OpenAPI_wlan_measurement_t *wlan_measurement)
{
    ...
    if (wlan_measurement == NULL) {
        ogs_error("OpenAPI_wlan_measurement_convertToJSON() failed [WlanMeasurement]");
        return NULL;
    }
    ...
    if (measurement_name_listList == NULL) {
        ogs_error("OpenAPI_wlan_measurement_convertToJSON() failed [measurement_name_list]");
        goto end;
    }
    OpenAPI_list_for_each(wlan_measurement->measurement_name_list, node) {
    ...
    if (wlan_measurement->wlan_rssi != OpenAPI_wlan_rssi_NULL) {
    if (cJSON_AddStringToObject(item, "wlanRssi", OpenAPI_wlan_rssi_ToString(wlan_measurement->wlan_rssi)) == NULL) {
        ogs_error("OpenAPI_wlan_measurement_convertToJSON() failed [wlan_rssi]");
        goto end;
    }
    }

    if (wlan_measurement->wlan_rtt != OpenAPI_wlan_rtt_NULL) {
    if (cJSON_AddStringToObject(item, "wlanRtt", OpenAPI_wlan_rtt_ToString(wlan_measurement->wlan_rtt)) == NULL) {
        ogs_error("OpenAPI_wlan_measurement_convertToJSON() failed [wlan_rtt]");
        goto end;
    }
    }
...
{
    OpenAPI_wlan_measurement_t *wlan_measurement_local_var = NULL;
    OpenAPI_lnode_t *node = NULL;
    ...
    measurement_name_list = cJSON_GetObjectItemCaseSensitive(wlan_measurementJSON, "measurementNameList");
    if (measurement_name_list) {
        cJSON *measurement_name_list_local = NULL;
        if (!cJSON_IsArray(measurement_name_list)) {
            ogs_error("OpenAPI_wlan_measurement_parseFromJSON() failed [measurement_name_list]");
            goto end;
        }
    ...
    wlan_rssi = cJSON_GetObjectItemCaseSensitive(wlan_measurementJSON, "wlanRssi");
    if (wlan_rssi) {
    if (!cJSON_IsString(wlan_rssi)) {
        ogs_error("OpenAPI_wlan_measurement_parseFromJSON() failed [wlan_rssi]");
        goto end;
    }
    wlan_rssiVariable = OpenAPI_wlan_rssi_FromString(wlan_rssi->valuestring);
    }

    wlan_rtt = cJSON_GetObjectItemCaseSensitive(wlan_measurementJSON, "wlanRtt");
    if (wlan_rtt) {
    if (!cJSON_IsString(wlan_rtt)) {
        ogs_error("OpenAPI_wlan_measurement_parseFromJSON() failed [wlan_rtt]");
        goto end;
    }
    wlan_rttVariable = OpenAPI_wlan_rtt_FromString(wlan_rtt->valuestring);
    }

    wlan_measurement_local_var = OpenAPI_wlan_measurement_create (
...
OpenAPI_wlan_measurement_t *OpenAPI_wlan_measurement_copy(OpenAPI_wlan_measurement_t *dst, OpenAPI_wlan_measurement_t *src)
{
    ...
    ogs_assert(src);
    item = OpenAPI_wlan_measurement_convertToJSON(src);
    if (!item) {
        ogs_error("OpenAPI_wlan_measurement_convertToJSON() failed");
        return NULL;
    }
    ...
    OpenAPI_wlan_measurement_free(dst);
    dst = OpenAPI_wlan_measurement_parseFromJSON(item);
    cJSON_Delete(item);
