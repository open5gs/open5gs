...
)
{
    ...
    af_routing_requirement_local_var->is_outcome_sub_null = is_outcome_sub_null;
    af_routing_requirement_local_var->outcome_sub = outcome_sub;
    af_routing_requirement_local_var->is_sim_conn_fail_sub_null = is_sim_conn_fail_sub_null;
    af_routing_requirement_local_var->sim_conn_fail_sub = sim_conn_fail_sub;
    ...
    af_routing_requirement_local_var->is_tfc_corre_info_null = is_tfc_corre_info_null;
    af_routing_requirement_local_var->tfc_corre_info = tfc_corre_info;
    af_routing_requirement_local_var->is_cand_dnai_ind = is_cand_dnai_ind;
    af_routing_requirement_local_var->cand_dnai_ind = cand_dnai_ind;
    af_routing_requirement_local_var->is_n6_delay_ind = is_n6_delay_ind;
    af_routing_requirement_local_var->n6_delay_ind = n6_delay_ind;
...
void OpenAPI_af_routing_requirement_free(OpenAPI_af_routing_requirement_t *af_routing_requirement)
{
    ...
    if (af_routing_requirement->outcome_sub) {
        OpenAPI_traff_route_req_outcome_event_free(af_routing_requirement->outcome_sub);
        af_routing_requirement->outcome_sub = NULL;
    }
    if (af_routing_requirement->sim_conn_fail_sub) {
        OpenAPI_sim_conn_fail_event_free(af_routing_requirement->sim_conn_fail_sub);
        af_routing_requirement->sim_conn_fail_sub = NULL;
    }
    ...
    if (af_routing_requirement->tfc_corre_info) {
        OpenAPI_traffic_correlation_info_free(af_routing_requirement->tfc_corre_info);
        af_routing_requirement->tfc_corre_info = NULL;
    }
...
cJSON *OpenAPI_af_routing_requirement_convertToJSON(OpenAPI_af_routing_requirement_t *af_routing_requirement)
{
    ...
    if (af_routing_requirement->outcome_sub) {
    cJSON *outcome_sub_local_JSON = OpenAPI_traff_route_req_outcome_event_convertToJSON(af_routing_requirement->outcome_sub);
    if (outcome_sub_local_JSON == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [outcome_sub]");
        goto end;
    }
    cJSON_AddItemToObject(item, "outcomeSub", outcome_sub_local_JSON);
    if (item->child == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [outcome_sub]");
        goto end;
    }
    } else if (af_routing_requirement->is_outcome_sub_null) {
        if (cJSON_AddNullToObject(item, "outcomeSub") == NULL) {
            ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [outcome_sub]");
            goto end;
        }
    }
    ...
    if (af_routing_requirement->sim_conn_fail_sub) {
    cJSON *sim_conn_fail_sub_local_JSON = OpenAPI_sim_conn_fail_event_convertToJSON(af_routing_requirement->sim_conn_fail_sub);
    if (sim_conn_fail_sub_local_JSON == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [sim_conn_fail_sub]");
        goto end;
    }
    cJSON_AddItemToObject(item, "simConnFailSub", sim_conn_fail_sub_local_JSON);
    if (item->child == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [sim_conn_fail_sub]");
        goto end;
    }
    } else if (af_routing_requirement->is_sim_conn_fail_sub_null) {
        if (cJSON_AddNullToObject(item, "simConnFailSub") == NULL) {
            ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [sim_conn_fail_sub]");
            goto end;
        }
    }
    ...
    if (af_routing_requirement->tfc_corre_info) {
    cJSON *tfc_corre_info_local_JSON = OpenAPI_traffic_correlation_info_convertToJSON(af_routing_requirement->tfc_corre_info);
    if (tfc_corre_info_local_JSON == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [tfc_corre_info]");
        goto end;
    }
    ...
    } else if (af_routing_requirement->is_tfc_corre_info_null) {
    ...
    if (af_routing_requirement->is_cand_dnai_ind) {
    if (cJSON_AddBoolToObject(item, "candDnaiInd", af_routing_requirement->cand_dnai_ind) == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [cand_dnai_ind]");
        goto end;
    }
    }

    if (af_routing_requirement->is_n6_delay_ind) {
    if (cJSON_AddBoolToObject(item, "n6DelayInd", af_routing_requirement->n6_delay_ind) == NULL) {
        ogs_error("OpenAPI_af_routing_requirement_convertToJSON() failed [n6_delay_ind]");
        goto end;
    }
    }
...
OpenAPI_af_routing_requirement_t *OpenAPI_af_routing_requirement_parseFromJSON(cJSON *af_routing_requirementJSON)
{
    ...
    cJSON *outcome_sub = NULL;
    OpenAPI_traff_route_req_outcome_event_t *outcome_sub_local_nonprim = NULL;
    cJSON *sim_conn_fail_sub = NULL;
    OpenAPI_sim_conn_fail_event_t *sim_conn_fail_sub_local_nonprim = NULL;
    ...
    outcome_sub = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON, "outcomeSub");
    if (outcome_sub) {
    if (!cJSON_IsNull(outcome_sub)) {
    outcome_sub_local_nonprim = OpenAPI_traff_route_req_outcome_event_parseFromJSON(outcome_sub);
    if (!outcome_sub_local_nonprim) {
        ogs_error("OpenAPI_traff_route_req_outcome_event_parseFromJSON failed [outcome_sub]");
        goto end;
    }
    }
    }

    sim_conn_fail_sub = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON, "simConnFailSub");
    if (sim_conn_fail_sub) {
    if (!cJSON_IsNull(sim_conn_fail_sub)) {
    sim_conn_fail_sub_local_nonprim = OpenAPI_sim_conn_fail_event_parseFromJSON(sim_conn_fail_sub);
    if (!sim_conn_fail_sub_local_nonprim) {
        ogs_error("OpenAPI_sim_conn_fail_event_parseFromJSON failed [sim_conn_fail_sub]");
        goto end;
    }
    }
    }
    ...
    tfc_corre_info = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON, "tfcCorreInfo");
    if (tfc_corre_info) {
    ...
    cand_dnai_ind = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON, "candDnaiInd");
    if (cand_dnai_ind) {
    if (!cJSON_IsBool(cand_dnai_ind)) {
        ogs_error("OpenAPI_af_routing_requirement_parseFromJSON() failed [cand_dnai_ind]");
        goto end;
    }
    }

    n6_delay_ind = cJSON_GetObjectItemCaseSensitive(af_routing_requirementJSON, "n6DelayInd");
    if (n6_delay_ind) {
    if (!cJSON_IsBool(n6_delay_ind)) {
        ogs_error("OpenAPI_af_routing_requirement_parseFromJSON() failed [n6_delay_ind]");
        goto end;
    }
    }
    ...
    af_routing_requirement_local_var = OpenAPI_af_routing_requirement_create (
        ...
        outcome_sub && cJSON_IsNull(outcome_sub) ? true : false,
        outcome_sub ? outcome_sub_local_nonprim : NULL,
        sim_conn_fail_sub && cJSON_IsNull(sim_conn_fail_sub) ? true : false,
        sim_conn_fail_sub ? sim_conn_fail_sub_local_nonprim : NULL,
    ...
    if (outcome_sub_local_nonprim) {
        OpenAPI_traff_route_req_outcome_event_free(outcome_sub_local_nonprim);
        outcome_sub_local_nonprim = NULL;
    }
    if (sim_conn_fail_sub_local_nonprim) {
        OpenAPI_sim_conn_fail_event_free(sim_conn_fail_sub_local_nonprim);
        sim_conn_fail_sub_local_nonprim = NULL;
    }
