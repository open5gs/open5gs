...
)
{
    ...
    hss_info_local_var->additional_diam_addresses = additional_diam_addresses;
...
void OpenAPI_hss_info_free(OpenAPI_hss_info_t *hss_info)
{
    ...
    if (hss_info->additional_diam_addresses) {
        OpenAPI_list_for_each(hss_info->additional_diam_addresses, node) {
            OpenAPI_network_node_diameter_address_free(node->data);
        }
        OpenAPI_list_free(hss_info->additional_diam_addresses);
        hss_info->additional_diam_addresses = NULL;
    }
    ogs_free(hss_info);
}
...
cJSON *OpenAPI_hss_info_convertToJSON(OpenAPI_hss_info_t *hss_info)
{
    ...
    if (additional_diam_addressesList == NULL) {
        ogs_error("OpenAPI_hss_info_convertToJSON() failed [additional_diam_addresses]");
        goto end;
    }
    OpenAPI_list_for_each(hss_info->additional_diam_addresses, node) {
...
OpenAPI_hss_info_t *OpenAPI_hss_info_parseFromJSON(cJSON *hss_infoJSON)
{
    ...
    additional_diam_addresses = cJSON_GetObjectItemCaseSensitive(hss_infoJSON, "additionalDiamAddresses");
    if (additional_diam_addresses) {
        cJSON *additional_diam_addresses_local = NULL;
        if (!cJSON_IsArray(additional_diam_addresses)) {
            ogs_error("OpenAPI_hss_info_parseFromJSON() failed [additional_diam_addresses]");
            goto end;
        }
