...
cJSON *OpenAPI_ml_model_inter_ind_convertToJSON(OpenAPI_ml_model_inter_ind_t *ml_model_inter_ind)
{
    ...
    if (ml_model_inter_ind->analytics_id == OpenAPI_nwdaf_event_NULL) {
        ogs_error("OpenAPI_ml_model_inter_ind_convertToJSON() failed [analytics_id]");
        return NULL;
    }
    if (cJSON_AddStringToObject(item, "analyticsId", OpenAPI_nwdaf_event_ToString(ml_model_inter_ind->analytics_id)) == NULL) {
        ogs_error("OpenAPI_ml_model_inter_ind_convertToJSON() failed [analytics_id]");
        goto end;
    }
...
OpenAPI_ml_model_inter_ind_t *OpenAPI_ml_model_inter_ind_parseFromJSON(cJSON *ml_model_inter_indJSON)
{
    ...
    analytics_id = cJSON_GetObjectItemCaseSensitive(ml_model_inter_indJSON, "analyticsId");
    if (!analytics_id) {
    ...
    vendor_list = cJSON_GetObjectItemCaseSensitive(ml_model_inter_indJSON, "vendorList");
    if (!vendor_list) {
