...
)
{
    ...
    additional_snssai_data_local_var->nsac_mode = nsac_mode;
    additional_snssai_data_local_var->valid_time_period = valid_time_period;
    additional_snssai_data_local_var->valid_time_period_list = valid_time_period_list;
    additional_snssai_data_local_var->is_dereg_inact_timer = is_dereg_inact_timer;
    additional_snssai_data_local_var->dereg_inact_timer = dereg_inact_timer;
    additional_snssai_data_local_var->is_on_demand = is_on_demand;
    additional_snssai_data_local_var->on_demand = on_demand;
...
void OpenAPI_additional_snssai_data_free(OpenAPI_additional_snssai_data_t *additional_snssai_data)
{
    ...
    if (additional_snssai_data->valid_time_period) {
        OpenAPI_valid_time_period_free(additional_snssai_data->valid_time_period);
        additional_snssai_data->valid_time_period = NULL;
    }
    if (additional_snssai_data->valid_time_period_list) {
        OpenAPI_list_for_each(additional_snssai_data->valid_time_period_list, node) {
            OpenAPI_recur_time_free(node->data);
        }
        OpenAPI_list_free(additional_snssai_data->valid_time_period_list);
        additional_snssai_data->valid_time_period_list = NULL;
    }
...
cJSON *OpenAPI_additional_snssai_data_convertToJSON(OpenAPI_additional_snssai_data_t *additional_snssai_data)
{
    ...
    if (additional_snssai_data->nsac_mode != OpenAPI_nsac_admission_mode_NULL) {
    if (cJSON_AddStringToObject(item, "nsacMode", OpenAPI_nsac_admission_mode_ToString(additional_snssai_data->nsac_mode)) == NULL) {
        ogs_error("OpenAPI_additional_snssai_data_convertToJSON() failed [nsac_mode]");
        goto end;
    }
    }

    if (additional_snssai_data->valid_time_period) {
    cJSON *valid_time_period_local_JSON = OpenAPI_valid_time_period_convertToJSON(additional_snssai_data->valid_time_period);
    if (valid_time_period_local_JSON == NULL) {
        ogs_error("OpenAPI_additional_snssai_data_convertToJSON() failed [valid_time_period]");
        goto end;
    }
    ...
    if (valid_time_period_listList == NULL) {
        ogs_error("OpenAPI_additional_snssai_data_convertToJSON() failed [valid_time_period_list]");
        goto end;
    }
    OpenAPI_list_for_each(additional_snssai_data->valid_time_period_list, node) {
    ...
    if (additional_snssai_data->is_dereg_inact_timer) {
    if (cJSON_AddNumberToObject(item, "deregInactTimer", additional_snssai_data->dereg_inact_timer) == NULL) {
        ogs_error("OpenAPI_additional_snssai_data_convertToJSON() failed [dereg_inact_timer]");
        goto end;
    }
    }

    if (additional_snssai_data->is_on_demand) {
    if (cJSON_AddBoolToObject(item, "onDemand", additional_snssai_data->on_demand) == NULL) {
        ogs_error("OpenAPI_additional_snssai_data_convertToJSON() failed [on_demand]");
        goto end;
    }
    }
...
OpenAPI_additional_snssai_data_t *OpenAPI_additional_snssai_data_parseFromJSON(cJSON *additional_snssai_dataJSON)
{
    ...
    nsac_mode = cJSON_GetObjectItemCaseSensitive(additional_snssai_dataJSON, "nsacMode");
    if (nsac_mode) {
    if (!cJSON_IsString(nsac_mode)) {
        ogs_error("OpenAPI_additional_snssai_data_parseFromJSON() failed [nsac_mode]");
        goto end;
    }
    nsac_modeVariable = OpenAPI_nsac_admission_mode_FromString(nsac_mode->valuestring);
    }

    valid_time_period = cJSON_GetObjectItemCaseSensitive(additional_snssai_dataJSON, "validTimePeriod");
    ...
    valid_time_period_list = cJSON_GetObjectItemCaseSensitive(additional_snssai_dataJSON, "validTimePeriodList");
    if (valid_time_period_list) {
        cJSON *valid_time_period_list_local = NULL;
        if (!cJSON_IsArray(valid_time_period_list)) {
            ogs_error("OpenAPI_additional_snssai_data_parseFromJSON() failed [valid_time_period_list]");
            goto end;
        }
        ...
        cJSON_ArrayForEach(valid_time_period_list_local, valid_time_period_list) {
            ...
            OpenAPI_recur_time_t *valid_time_period_listItem = OpenAPI_recur_time_parseFromJSON(valid_time_period_list_local);
    ...
    dereg_inact_timer = cJSON_GetObjectItemCaseSensitive(additional_snssai_dataJSON, "deregInactTimer");
    if (dereg_inact_timer) {
    if (!cJSON_IsNumber(dereg_inact_timer)) {
        ogs_error("OpenAPI_additional_snssai_data_parseFromJSON() failed [dereg_inact_timer]");
        goto end;
    }
    }

    on_demand = cJSON_GetObjectItemCaseSensitive(additional_snssai_dataJSON, "onDemand");
    if (on_demand) {
    if (!cJSON_IsBool(on_demand)) {
        ogs_error("OpenAPI_additional_snssai_data_parseFromJSON() failed [on_demand]");
        goto end;
    }
    }
