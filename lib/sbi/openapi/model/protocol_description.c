...
{
    OpenAPI_protocol_description_t *protocol_description_local_var = ogs_malloc(sizeof(OpenAPI_protocol_description_t));
    ogs_assert(protocol_description_local_var);

    protocol_description_local_var->transport_proto = transport_proto;
    protocol_description_local_var->rtp_header_ext_info = rtp_header_ext_info;
    protocol_description_local_var->add_rtp_header_ext_info = add_rtp_header_ext_info;
    protocol_description_local_var->rtp_payload_info_list = rtp_payload_info_list;
    protocol_description_local_var->mri_transfer_info = mri_transfer_info;

    return protocol_description_local_var;
}
...
void OpenAPI_protocol_description_free(OpenAPI_protocol_description_t *protocol_description)
{
    ...
    if (NULL == protocol_description) {
        return;
    }
    if (protocol_description->rtp_header_ext_info) {
        OpenAPI_rtp_header_ext_info_free(protocol_description->rtp_header_ext_info);
        protocol_description->rtp_header_ext_info = NULL;
    }
    if (protocol_description->add_rtp_header_ext_info) {
        OpenAPI_list_for_each(protocol_description->add_rtp_header_ext_info, node) {
            OpenAPI_rtp_header_ext_info_free(node->data);
        }
        OpenAPI_list_free(protocol_description->add_rtp_header_ext_info);
        protocol_description->add_rtp_header_ext_info = NULL;
    }
    if (protocol_description->rtp_payload_info_list) {
        OpenAPI_list_for_each(protocol_description->rtp_payload_info_list, node) {
            OpenAPI_rtp_payload_info_free(node->data);
        }
        OpenAPI_list_free(protocol_description->rtp_payload_info_list);
        protocol_description->rtp_payload_info_list = NULL;
    }
    ogs_free(protocol_description);
}
...
cJSON *OpenAPI_protocol_description_convertToJSON(OpenAPI_protocol_description_t *protocol_description)
{
    ...
    if (protocol_description == NULL) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed [ProtocolDescription]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (protocol_description->transport_proto != OpenAPI_media_transport_proto_NULL) {
    if (cJSON_AddStringToObject(item, "transportProto", OpenAPI_media_transport_proto_ToString(protocol_description->transport_proto)) == NULL) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed [transport_proto]");
        goto end;
    }
    }

    if (protocol_description->rtp_header_ext_info) {
    cJSON *rtp_header_ext_info_local_JSON = OpenAPI_rtp_header_ext_info_convertToJSON(protocol_description->rtp_header_ext_info);
    if (rtp_header_ext_info_local_JSON == NULL) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed [rtp_header_ext_info]");
        goto end;
    }
    ...
    if (add_rtp_header_ext_infoList == NULL) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed [add_rtp_header_ext_info]");
        goto end;
    }
    OpenAPI_list_for_each(protocol_description->add_rtp_header_ext_info, node) {
    ...
    if (rtp_payload_info_listList == NULL) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed [rtp_payload_info_list]");
        goto end;
    }
    OpenAPI_list_for_each(protocol_description->rtp_payload_info_list, node) {
    ...
    if (protocol_description->mri_transfer_info != OpenAPI_mri_transfer_method_NULL) {
    if (cJSON_AddStringToObject(item, "mriTransferInfo", OpenAPI_mri_transfer_method_ToString(protocol_description->mri_transfer_info)) == NULL) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed [mri_transfer_info]");
        goto end;
    }
    }
...
{
    OpenAPI_protocol_description_t *protocol_description_local_var = NULL;
    OpenAPI_lnode_t *node = NULL;
    cJSON *transport_proto = NULL;
    OpenAPI_media_transport_proto_e transport_protoVariable = 0;
    cJSON *rtp_header_ext_info = NULL;
    OpenAPI_rtp_header_ext_info_t *rtp_header_ext_info_local_nonprim = NULL;
    ...
    transport_proto = cJSON_GetObjectItemCaseSensitive(protocol_descriptionJSON, "transportProto");
    if (transport_proto) {
    if (!cJSON_IsString(transport_proto)) {
        ogs_error("OpenAPI_protocol_description_parseFromJSON() failed [transport_proto]");
        goto end;
    }
    transport_protoVariable = OpenAPI_media_transport_proto_FromString(transport_proto->valuestring);
    }

    rtp_header_ext_info = cJSON_GetObjectItemCaseSensitive(protocol_descriptionJSON, "rtpHeaderExtInfo");
    if (rtp_header_ext_info) {
    rtp_header_ext_info_local_nonprim = OpenAPI_rtp_header_ext_info_parseFromJSON(rtp_header_ext_info);
    if (!rtp_header_ext_info_local_nonprim) {
        ogs_error("OpenAPI_rtp_header_ext_info_parseFromJSON failed [rtp_header_ext_info]");
        goto end;
    }
    }

    add_rtp_header_ext_info = cJSON_GetObjectItemCaseSensitive(protocol_descriptionJSON, "addRtpHeaderExtInfo");
    if (add_rtp_header_ext_info) {
        cJSON *add_rtp_header_ext_info_local = NULL;
        if (!cJSON_IsArray(add_rtp_header_ext_info)) {
            ogs_error("OpenAPI_protocol_description_parseFromJSON() failed [add_rtp_header_ext_info]");
            goto end;
        }
    ...
    rtp_payload_info_list = cJSON_GetObjectItemCaseSensitive(protocol_descriptionJSON, "rtpPayloadInfoList");
    if (rtp_payload_info_list) {
        cJSON *rtp_payload_info_list_local = NULL;
        if (!cJSON_IsArray(rtp_payload_info_list)) {
            ogs_error("OpenAPI_protocol_description_parseFromJSON() failed [rtp_payload_info_list]");
            goto end;
        }
    ...
    mri_transfer_info = cJSON_GetObjectItemCaseSensitive(protocol_descriptionJSON, "mriTransferInfo");
    if (mri_transfer_info) {
    if (!cJSON_IsString(mri_transfer_info)) {
        ogs_error("OpenAPI_protocol_description_parseFromJSON() failed [mri_transfer_info]");
        goto end;
    }
    mri_transfer_infoVariable = OpenAPI_mri_transfer_method_FromString(mri_transfer_info->valuestring);
    }

    protocol_description_local_var = OpenAPI_protocol_description_create (
    ...
    if (rtp_header_ext_info_local_nonprim) {
        OpenAPI_rtp_header_ext_info_free(rtp_header_ext_info_local_nonprim);
...
OpenAPI_protocol_description_t *OpenAPI_protocol_description_copy(OpenAPI_protocol_description_t *dst, OpenAPI_protocol_description_t *src)
{
    ...
    ogs_assert(src);
    item = OpenAPI_protocol_description_convertToJSON(src);
    if (!item) {
        ogs_error("OpenAPI_protocol_description_convertToJSON() failed");
        return NULL;
    }
    ...
    OpenAPI_protocol_description_free(dst);
    dst = OpenAPI_protocol_description_parseFromJSON(item);
    cJSON_Delete(item);
