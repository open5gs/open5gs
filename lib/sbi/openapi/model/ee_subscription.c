...
)
{
    ...
    ee_subscription_local_var->is_udr_restart_ind = is_udr_restart_ind;
    ee_subscription_local_var->udr_restart_ind = udr_restart_ind;
    ee_subscription_local_var->last_synchronization_time = last_synchronization_time;
    ee_subscription_local_var->subscription_update_callback_uri = subscription_update_callback_uri;
...
void OpenAPI_ee_subscription_free(OpenAPI_ee_subscription_t *ee_subscription)
{
    ...
    if (ee_subscription->last_synchronization_time) {
        ogs_free(ee_subscription->last_synchronization_time);
        ee_subscription->last_synchronization_time = NULL;
    }
    if (ee_subscription->subscription_update_callback_uri) {
        ogs_free(ee_subscription->subscription_update_callback_uri);
        ee_subscription->subscription_update_callback_uri = NULL;
    }
...
cJSON *OpenAPI_ee_subscription_convertToJSON(OpenAPI_ee_subscription_t *ee_subscription)
{
    ...
    if (ee_subscription->is_udr_restart_ind) {
    if (cJSON_AddBoolToObject(item, "udrRestartInd", ee_subscription->udr_restart_ind) == NULL) {
        ogs_error("OpenAPI_ee_subscription_convertToJSON() failed [udr_restart_ind]");
        goto end;
    }
    }

    if (ee_subscription->last_synchronization_time) {
    if (cJSON_AddStringToObject(item, "lastSynchronizationTime", ee_subscription->last_synchronization_time) == NULL) {
        ogs_error("OpenAPI_ee_subscription_convertToJSON() failed [last_synchronization_time]");
        goto end;
    }
    }

    if (ee_subscription->subscription_update_callback_uri) {
    if (cJSON_AddStringToObject(item, "subscriptionUpdateCallbackUri", ee_subscription->subscription_update_callback_uri) == NULL) {
        ogs_error("OpenAPI_ee_subscription_convertToJSON() failed [subscription_update_callback_uri]");
        goto end;
    }
    }
...
OpenAPI_ee_subscription_t *OpenAPI_ee_subscription_parseFromJSON(cJSON *ee_subscriptionJSON)
{
    ...
    udr_restart_ind = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON, "udrRestartInd");
    if (udr_restart_ind) {
    if (!cJSON_IsBool(udr_restart_ind)) {
        ogs_error("OpenAPI_ee_subscription_parseFromJSON() failed [udr_restart_ind]");
        goto end;
    }
    }

    last_synchronization_time = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON, "lastSynchronizationTime");
    if (last_synchronization_time) {
    if (!cJSON_IsString(last_synchronization_time) && !cJSON_IsNull(last_synchronization_time)) {
        ogs_error("OpenAPI_ee_subscription_parseFromJSON() failed [last_synchronization_time]");
        goto end;
    }
    }

    subscription_update_callback_uri = cJSON_GetObjectItemCaseSensitive(ee_subscriptionJSON, "subscriptionUpdateCallbackUri");
    ...
    if (!cJSON_IsString(subscription_update_callback_uri) && !cJSON_IsNull(subscription_update_callback_uri)) {
        ogs_error("OpenAPI_ee_subscription_parseFromJSON() failed [subscription_update_callback_uri]");
        goto end;
    }
    }
