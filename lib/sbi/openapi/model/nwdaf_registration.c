...
cJSON *OpenAPI_nwdaf_registration_convertToJSON(OpenAPI_nwdaf_registration_t *nwdaf_registration)
{
    ...
    if (nwdaf_registration->analytics_ids == OpenAPI_event_id_NULL) {
...
OpenAPI_nwdaf_registration_t *OpenAPI_nwdaf_registration_parseFromJSON(cJSON *nwdaf_registrationJSON)
{
    ...
    if (!analytics_ids) {
        ...
        if (analytics_idsList->count == 0) {
            ogs_error("OpenAPI_nwdaf_registration_parseFromJSON() failed: Expected analytics_idsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
