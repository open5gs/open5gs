...
)
{
    ...
    trace_data_local_var->trace_reporting_consumer_uri = trace_reporting_consumer_uri;
    trace_data_local_var->interface_list = interface_list;
    trace_data_local_var->job_type = job_type;
...
void OpenAPI_trace_data_free(OpenAPI_trace_data_t *trace_data)
{
    ...
    if (trace_data->trace_reporting_consumer_uri) {
        ogs_free(trace_data->trace_reporting_consumer_uri);
...
cJSON *OpenAPI_trace_data_convertToJSON(OpenAPI_trace_data_t *trace_data)
{
    ...
    if (trace_data->trace_reporting_consumer_uri) {
    if (cJSON_AddStringToObject(item, "traceReportingConsumerUri", trace_data->trace_reporting_consumer_uri) == NULL) {
        ogs_error("OpenAPI_trace_data_convertToJSON() failed [trace_reporting_consumer_uri]");
        goto end;
    }
    }
    ...
    if (trace_data->job_type != OpenAPI_job_type_NULL) {
    if (cJSON_AddStringToObject(item, "jobType", OpenAPI_job_type_ToString(trace_data->job_type)) == NULL) {
        ogs_error("OpenAPI_trace_data_convertToJSON() failed [job_type]");
        goto end;
    }
    }
...
OpenAPI_trace_data_t *OpenAPI_trace_data_parseFromJSON(cJSON *trace_dataJSON)
{
    ...
    trace_reporting_consumer_uri = cJSON_GetObjectItemCaseSensitive(trace_dataJSON, "traceReportingConsumerUri");
    if (trace_reporting_consumer_uri) {
    if (!cJSON_IsString(trace_reporting_consumer_uri) && !cJSON_IsNull(trace_reporting_consumer_uri)) {
        ogs_error("OpenAPI_trace_data_parseFromJSON() failed [trace_reporting_consumer_uri]");
        goto end;
    }
    }
    ...
    job_type = cJSON_GetObjectItemCaseSensitive(trace_dataJSON, "jobType");
    if (job_type) {
    if (!cJSON_IsString(job_type)) {
        ogs_error("OpenAPI_trace_data_parseFromJSON() failed [job_type]");
        goto end;
    }
