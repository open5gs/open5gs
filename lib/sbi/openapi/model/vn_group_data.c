...
)
{
    ...
    vn_group_data_local_var->is_secondary_auth = is_secondary_auth;
    vn_group_data_local_var->secondary_auth = secondary_auth;
    vn_group_data_local_var->is_dn_aaa_ip_address_allocation = is_dn_aaa_ip_address_allocation;
    vn_group_data_local_var->dn_aaa_ip_address_allocation = dn_aaa_ip_address_allocation;
    vn_group_data_local_var->dn_aaa_address = dn_aaa_address;
    vn_group_data_local_var->additional_dn_aaa_addresses = additional_dn_aaa_addresses;
    vn_group_data_local_var->dn_aaa_fqdn = dn_aaa_fqdn;
    vn_group_data_local_var->is__5g_vn_group_communication_ind = is__5g_vn_group_communication_ind;
    vn_group_data_local_var->_5g_vn_group_communication_ind = _5g_vn_group_communication_ind;
    vn_group_data_local_var->_5g_vn_group_communication_type = _5g_vn_group_communication_type;
    vn_group_data_local_var->max_group_data_rate = max_group_data_rate;
...
void OpenAPI_vn_group_data_free(OpenAPI_vn_group_data_t *vn_group_data)
{
    ...
    if (vn_group_data->dn_aaa_address) {
        OpenAPI_ip_address_free(vn_group_data->dn_aaa_address);
        vn_group_data->dn_aaa_address = NULL;
    }
    if (vn_group_data->additional_dn_aaa_addresses) {
        OpenAPI_list_for_each(vn_group_data->additional_dn_aaa_addresses, node) {
            OpenAPI_ip_address_free(node->data);
        }
        OpenAPI_list_free(vn_group_data->additional_dn_aaa_addresses);
        vn_group_data->additional_dn_aaa_addresses = NULL;
    }
    if (vn_group_data->dn_aaa_fqdn) {
        ogs_free(vn_group_data->dn_aaa_fqdn);
        vn_group_data->dn_aaa_fqdn = NULL;
    }
    if (vn_group_data->max_group_data_rate) {
        OpenAPI_max_group_data_rate_free(vn_group_data->max_group_data_rate);
        vn_group_data->max_group_data_rate = NULL;
    }
...
cJSON *OpenAPI_vn_group_data_convertToJSON(OpenAPI_vn_group_data_t *vn_group_data)
{
    ...
    if (vn_group_data->is_secondary_auth) {
    if (cJSON_AddBoolToObject(item, "secondaryAuth", vn_group_data->secondary_auth) == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [secondary_auth]");
        goto end;
    }
    }

    if (vn_group_data->is_dn_aaa_ip_address_allocation) {
    if (cJSON_AddBoolToObject(item, "dnAaaIpAddressAllocation", vn_group_data->dn_aaa_ip_address_allocation) == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [dn_aaa_ip_address_allocation]");
        goto end;
    }
    }

    if (vn_group_data->dn_aaa_address) {
    cJSON *dn_aaa_address_local_JSON = OpenAPI_ip_address_convertToJSON(vn_group_data->dn_aaa_address);
    if (dn_aaa_address_local_JSON == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [dn_aaa_address]");
        goto end;
    }
    ...
    if (additional_dn_aaa_addressesList == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [additional_dn_aaa_addresses]");
        goto end;
    }
    OpenAPI_list_for_each(vn_group_data->additional_dn_aaa_addresses, node) {
    ...
    if (vn_group_data->dn_aaa_fqdn) {
    if (cJSON_AddStringToObject(item, "dnAaaFqdn", vn_group_data->dn_aaa_fqdn) == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [dn_aaa_fqdn]");
        goto end;
    }
    }

    if (vn_group_data->is__5g_vn_group_communication_ind) {
    if (cJSON_AddBoolToObject(item, "5gVnGroupCommunicationInd", vn_group_data->_5g_vn_group_communication_ind) == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [_5g_vn_group_communication_ind]");
        goto end;
    }
    }

    if (vn_group_data->_5g_vn_group_communication_type != OpenAPI_model_5_gvn_group_communication_type_NULL) {
    if (cJSON_AddStringToObject(item, "5gVnGroupCommunicationType", OpenAPI_model_5_gvn_group_communication_type_ToString(vn_group_data->_5g_vn_group_communication_type)) == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [_5g_vn_group_communication_type]");
        goto end;
    }
    }

    if (vn_group_data->max_group_data_rate) {
    cJSON *max_group_data_rate_local_JSON = OpenAPI_max_group_data_rate_convertToJSON(vn_group_data->max_group_data_rate);
    if (max_group_data_rate_local_JSON == NULL) {
        ogs_error("OpenAPI_vn_group_data_convertToJSON() failed [max_group_data_rate]");
        goto end;
    }
...
OpenAPI_vn_group_data_t *OpenAPI_vn_group_data_parseFromJSON(cJSON *vn_group_dataJSON)
{
    ...
    secondary_auth = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "secondaryAuth");
    if (secondary_auth) {
    if (!cJSON_IsBool(secondary_auth)) {
        ogs_error("OpenAPI_vn_group_data_parseFromJSON() failed [secondary_auth]");
        goto end;
    }
    }

    dn_aaa_ip_address_allocation = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "dnAaaIpAddressAllocation");
    if (dn_aaa_ip_address_allocation) {
    if (!cJSON_IsBool(dn_aaa_ip_address_allocation)) {
        ogs_error("OpenAPI_vn_group_data_parseFromJSON() failed [dn_aaa_ip_address_allocation]");
        goto end;
    }
    }

    dn_aaa_address = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "dnAaaAddress");
    if (dn_aaa_address) {
    ...
    additional_dn_aaa_addresses = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "additionalDnAaaAddresses");
    if (additional_dn_aaa_addresses) {
        cJSON *additional_dn_aaa_addresses_local = NULL;
        if (!cJSON_IsArray(additional_dn_aaa_addresses)) {
            ogs_error("OpenAPI_vn_group_data_parseFromJSON() failed [additional_dn_aaa_addresses]");
            goto end;
        }
    ...
    dn_aaa_fqdn = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "dnAaaFqdn");
    if (dn_aaa_fqdn) {
    if (!cJSON_IsString(dn_aaa_fqdn) && !cJSON_IsNull(dn_aaa_fqdn)) {
        ogs_error("OpenAPI_vn_group_data_parseFromJSON() failed [dn_aaa_fqdn]");
        goto end;
    }
    }

    _5g_vn_group_communication_ind = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "5gVnGroupCommunicationInd");
    if (_5g_vn_group_communication_ind) {
    if (!cJSON_IsBool(_5g_vn_group_communication_ind)) {
        ogs_error("OpenAPI_vn_group_data_parseFromJSON() failed [_5g_vn_group_communication_ind]");
        goto end;
    }
    }

    _5g_vn_group_communication_type = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "5gVnGroupCommunicationType");
    if (_5g_vn_group_communication_type) {
    if (!cJSON_IsString(_5g_vn_group_communication_type)) {
        ogs_error("OpenAPI_vn_group_data_parseFromJSON() failed [_5g_vn_group_communication_type]");
        goto end;
    }
    ...
    max_group_data_rate = cJSON_GetObjectItemCaseSensitive(vn_group_dataJSON, "maxGroupDataRate");
