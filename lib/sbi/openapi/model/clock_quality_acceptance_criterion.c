...
{
    OpenAPI_clock_quality_acceptance_criterion_t *clock_quality_acceptance_criterion_local_var = ogs_malloc(sizeof(OpenAPI_clock_quality_acceptance_criterion_t));
    ogs_assert(clock_quality_acceptance_criterion_local_var);

    clock_quality_acceptance_criterion_local_var->synchronization_state = synchronization_state;
    clock_quality_acceptance_criterion_local_var->clock_quality = clock_quality;
    clock_quality_acceptance_criterion_local_var->parent_time_source = parent_time_source;
...
void OpenAPI_clock_quality_acceptance_criterion_free(OpenAPI_clock_quality_acceptance_criterion_t *clock_quality_acceptance_criterion)
{
    ...
    if (clock_quality_acceptance_criterion->synchronization_state) {
        OpenAPI_list_free(clock_quality_acceptance_criterion->synchronization_state);
        clock_quality_acceptance_criterion->synchronization_state = NULL;
    }
    if (clock_quality_acceptance_criterion->clock_quality) {
        OpenAPI_clock_quality_free(clock_quality_acceptance_criterion->clock_quality);
        clock_quality_acceptance_criterion->clock_quality = NULL;
    }
    if (clock_quality_acceptance_criterion->parent_time_source) {
        OpenAPI_list_free(clock_quality_acceptance_criterion->parent_time_source);
        clock_quality_acceptance_criterion->parent_time_source = NULL;
    }
    ogs_free(clock_quality_acceptance_criterion);
}
...
cJSON *OpenAPI_clock_quality_acceptance_criterion_convertToJSON(OpenAPI_clock_quality_acceptance_criterion_t *clock_quality_acceptance_criterion)
{
    ...
    if (clock_quality_acceptance_criterion == NULL) {
        ogs_error("OpenAPI_clock_quality_acceptance_criterion_convertToJSON() failed [ClockQualityAcceptanceCriterion]");
        return NULL;
    }

    item = cJSON_CreateObject();
    if (clock_quality_acceptance_criterion->synchronization_state != OpenAPI_synchronization_state_NULL) {
    cJSON *synchronization_stateList = cJSON_AddArrayToObject(item, "synchronizationState");
    if (synchronization_stateList == NULL) {
        ogs_error("OpenAPI_clock_quality_acceptance_criterion_convertToJSON() failed [synchronization_state]");
        goto end;
    }
    OpenAPI_list_for_each(clock_quality_acceptance_criterion->synchronization_state, node) {
    ...
    if (clock_quality_acceptance_criterion->clock_quality) {
    cJSON *clock_quality_local_JSON = OpenAPI_clock_quality_convertToJSON(clock_quality_acceptance_criterion->clock_quality);
    if (clock_quality_local_JSON == NULL) {
        ogs_error("OpenAPI_clock_quality_acceptance_criterion_convertToJSON() failed [clock_quality]");
        goto end;
    }
    ...
    if (clock_quality_acceptance_criterion->parent_time_source != OpenAPI_time_source_NULL) {
    cJSON *parent_time_sourceList = cJSON_AddArrayToObject(item, "parentTimeSource");
    if (parent_time_sourceList == NULL) {
        ogs_error("OpenAPI_clock_quality_acceptance_criterion_convertToJSON() failed [parent_time_source]");
        goto end;
    }
    OpenAPI_list_for_each(clock_quality_acceptance_criterion->parent_time_source, node) {
...
{
    OpenAPI_clock_quality_acceptance_criterion_t *clock_quality_acceptance_criterion_local_var = NULL;
    OpenAPI_lnode_t *node = NULL;
    cJSON *synchronization_state = NULL;
    OpenAPI_list_t *synchronization_stateList = NULL;
    cJSON *clock_quality = NULL;
    OpenAPI_clock_quality_t *clock_quality_local_nonprim = NULL;
    cJSON *parent_time_source = NULL;
    OpenAPI_list_t *parent_time_sourceList = NULL;
    synchronization_state = cJSON_GetObjectItemCaseSensitive(clock_quality_acceptance_criterionJSON, "synchronizationState");
    if (synchronization_state) {
        cJSON *synchronization_state_local = NULL;
        if (!cJSON_IsArray(synchronization_state)) {
            ogs_error("OpenAPI_clock_quality_acceptance_criterion_parseFromJSON() failed [synchronization_state]");
            goto end;
        }
        ...
        if (synchronization_stateList->count == 0) {
            ogs_error("OpenAPI_clock_quality_acceptance_criterion_parseFromJSON() failed: Expected synchronization_stateList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }

    clock_quality = cJSON_GetObjectItemCaseSensitive(clock_quality_acceptance_criterionJSON, "clockQuality");
    if (clock_quality) {
    clock_quality_local_nonprim = OpenAPI_clock_quality_parseFromJSON(clock_quality);
    if (!clock_quality_local_nonprim) {
        ogs_error("OpenAPI_clock_quality_parseFromJSON failed [clock_quality]");
        goto end;
    }
    }

    parent_time_source = cJSON_GetObjectItemCaseSensitive(clock_quality_acceptance_criterionJSON, "parentTimeSource");
    if (parent_time_source) {
        cJSON *parent_time_source_local = NULL;
        if (!cJSON_IsArray(parent_time_source)) {
            ogs_error("OpenAPI_clock_quality_acceptance_criterion_parseFromJSON() failed [parent_time_source]");
            goto end;
        }
        ...
        if (parent_time_sourceList->count == 0) {
            ogs_error("OpenAPI_clock_quality_acceptance_criterion_parseFromJSON() failed: Expected parent_time_sourceList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }

    clock_quality_acceptance_criterion_local_var = OpenAPI_clock_quality_acceptance_criterion_create (
    ...
    if (clock_quality_local_nonprim) {
        OpenAPI_clock_quality_free(clock_quality_local_nonprim);
...
OpenAPI_clock_quality_acceptance_criterion_t *OpenAPI_clock_quality_acceptance_criterion_copy(OpenAPI_clock_quality_acceptance_criterion_t *dst, OpenAPI_clock_quality_acceptance_criterion_t *src)
{
    ...
    ogs_assert(src);
    item = OpenAPI_clock_quality_acceptance_criterion_convertToJSON(src);
    if (!item) {
        ogs_error("OpenAPI_clock_quality_acceptance_criterion_convertToJSON() failed");
        return NULL;
    }
    ...
    OpenAPI_clock_quality_acceptance_criterion_free(dst);
    dst = OpenAPI_clock_quality_acceptance_criterion_parseFromJSON(item);
    cJSON_Delete(item);
