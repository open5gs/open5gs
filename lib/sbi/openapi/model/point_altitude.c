...
cJSON *OpenAPI_point_altitude_convertToJSON(OpenAPI_point_altitude_t *point_altitude)
{
    ...
    if (point_altitude->shape == OpenAPI_supported_gad_shapes_NULL) {
        ogs_error("OpenAPI_point_altitude_convertToJSON() failed [shape]");
        return NULL;
    }
    if (cJSON_AddStringToObject(item, "shape", OpenAPI_supported_gad_shapes_ToString(point_altitude->shape)) == NULL) {
        ogs_error("OpenAPI_point_altitude_convertToJSON() failed [shape]");
        goto end;
    }
