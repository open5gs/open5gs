...
)
{
    ...
    data_change_notify_local_var->additional_data = additional_data;
...
void OpenAPI_data_change_notify_free(OpenAPI_data_change_notify_t *data_change_notify)
{
    ...
    if (data_change_notify->additional_data) {
        OpenAPI_list_for_each(data_change_notify->additional_data, node) {
            OpenAPI_map_t *localKeyValue = (OpenAPI_map_t*)node->data;
            ogs_free(localKeyValue->key);
            cJSON_Delete(localKeyValue->value);
            OpenAPI_map_free(localKeyValue);
        }
        OpenAPI_list_free(data_change_notify->additional_data);
        data_change_notify->additional_data = NULL;
    }
...
cJSON *OpenAPI_data_change_notify_convertToJSON(OpenAPI_data_change_notify_t *data_change_notify)
{
    ...
    if (data_change_notify->additional_data) {
    cJSON *additional_data = cJSON_AddObjectToObject(item, "additionalData");
    if (additional_data == NULL) {
        ogs_error("OpenAPI_data_change_notify_convertToJSON() failed [additional_data]");
        goto end;
    }
    cJSON *localMapObject = additional_data;
    if (data_change_notify->additional_data) {
        OpenAPI_list_for_each(data_change_notify->additional_data, node) {
            OpenAPI_map_t *localKeyValue = (OpenAPI_map_t*)node->data;
            ...
            cJSON *itemLocal = localKeyValue->value ?
                cJSON_Duplicate(localKeyValue->value, 1) :
                cJSON_CreateNull();
            if (itemLocal == NULL) {
                ogs_error("OpenAPI_data_change_notify_convertToJSON() failed [inner]");
                goto end;
            }
...
OpenAPI_data_change_notify_t *OpenAPI_data_change_notify_parseFromJSON(cJSON *data_change_notifyJSON)
{
    ...
    cJSON *additional_data = NULL;
    OpenAPI_list_t *additional_dataList = NULL;
    ...
    if (!sdm_subscription_local_nonprim) {
        ogs_error("OpenAPI_sdm_subscription_parseFromJSON failed [sdm_subscription]");
        goto end;
    }
    }
    ...
    if (additional_sdm_subscriptions) {
        ...
        cJSON_ArrayForEach(additional_sdm_subscriptions_local, additional_sdm_subscriptions) {
            ...
            OpenAPI_sdm_subscription_t *additional_sdm_subscriptionsItem = OpenAPI_sdm_subscription_parseFromJSON(additional_sdm_subscriptions_local);
    ...
    additional_data = cJSON_GetObjectItemCaseSensitive(data_change_notifyJSON, "additionalData");
    if (additional_data) {
        cJSON *additional_data_local_map = NULL;
        if (!cJSON_IsObject(additional_data) && !cJSON_IsNull(additional_data)) {
            ogs_error("OpenAPI_data_change_notify_parseFromJSON() failed [additional_data]");
            goto end;
        }
        if (cJSON_IsObject(additional_data)) {
            additional_dataList = OpenAPI_list_create();
            OpenAPI_map_t *localMapKeyPair = NULL;
            cJSON_ArrayForEach(additional_data_local_map, additional_data) {
                cJSON *localMapObject = additional_data_local_map;
                if (cJSON_IsObject(localMapObject)) {
                    localMapKeyPair = OpenAPI_map_create(
                        ogs_strdup(localMapObject->string), cJSON_Duplicate(localMapObject, 1));
                } else if (cJSON_IsNull(localMapObject)) {
                    localMapKeyPair = OpenAPI_map_create(ogs_strdup(localMapObject->string), NULL);
                } else {
                    ogs_error("OpenAPI_data_change_notify_parseFromJSON() failed [inner]");
                    goto end;
                }
                ...
                OpenAPI_list_add(additional_dataList, localMapKeyPair);
            }
        }
    }
    ...
    data_change_notify_local_var = OpenAPI_data_change_notify_create (
        ...
        additional_data ? additional_dataList : NULL
    );
    ...
    if (sdm_subscription_local_nonprim) {
        OpenAPI_sdm_subscription_free(sdm_subscription_local_nonprim);
    ...
    if (additional_dataList) {
        OpenAPI_list_for_each(additional_dataList, node) {
            OpenAPI_map_t *localKeyValue = (OpenAPI_map_t*)node->data;
            ogs_free(localKeyValue->key);
            cJSON_Delete(localKeyValue->value);
            OpenAPI_map_free(localKeyValue);
        }
        OpenAPI_list_free(additional_dataList);
        additional_dataList = NULL;
    }
    return NULL;
}
