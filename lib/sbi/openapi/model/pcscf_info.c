...
)
{
    ...
    pcscf_info_local_var->supi_ranges = supi_ranges;
    pcscf_info_local_var->gpsi_ranges = gpsi_ranges;
    pcscf_info_local_var->group_id = group_id;
    pcscf_info_local_var->serving_plmns = serving_plmns;
...
void OpenAPI_pcscf_info_free(OpenAPI_pcscf_info_t *pcscf_info)
{
    ...
    if (pcscf_info->supi_ranges) {
        OpenAPI_list_for_each(pcscf_info->supi_ranges, node) {
            OpenAPI_supi_range_free(node->data);
        }
        OpenAPI_list_free(pcscf_info->supi_ranges);
        pcscf_info->supi_ranges = NULL;
    }
    if (pcscf_info->gpsi_ranges) {
        OpenAPI_list_for_each(pcscf_info->gpsi_ranges, node) {
            OpenAPI_identity_range_free(node->data);
        }
        OpenAPI_list_free(pcscf_info->gpsi_ranges);
        pcscf_info->gpsi_ranges = NULL;
    }
    if (pcscf_info->group_id) {
        ogs_free(pcscf_info->group_id);
        pcscf_info->group_id = NULL;
    }
    if (pcscf_info->serving_plmns) {
        OpenAPI_list_for_each(pcscf_info->serving_plmns, node) {
            OpenAPI_plmn_id_free(node->data);
        }
        OpenAPI_list_free(pcscf_info->serving_plmns);
        pcscf_info->serving_plmns = NULL;
    }
    ogs_free(pcscf_info);
}
...
cJSON *OpenAPI_pcscf_info_convertToJSON(OpenAPI_pcscf_info_t *pcscf_info)
{
    ...
    if (supi_rangesList == NULL) {
        ogs_error("OpenAPI_pcscf_info_convertToJSON() failed [supi_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(pcscf_info->supi_ranges, node) {
    ...
    if (gpsi_rangesList == NULL) {
        ogs_error("OpenAPI_pcscf_info_convertToJSON() failed [gpsi_ranges]");
        goto end;
    }
    OpenAPI_list_for_each(pcscf_info->gpsi_ranges, node) {
    ...
    if (pcscf_info->group_id) {
    if (cJSON_AddStringToObject(item, "groupId", pcscf_info->group_id) == NULL) {
        ogs_error("OpenAPI_pcscf_info_convertToJSON() failed [group_id]");
        goto end;
    }
    }
    ...
    if (serving_plmnsList == NULL) {
        ogs_error("OpenAPI_pcscf_info_convertToJSON() failed [serving_plmns]");
        goto end;
    }
    OpenAPI_list_for_each(pcscf_info->serving_plmns, node) {
...
OpenAPI_pcscf_info_t *OpenAPI_pcscf_info_parseFromJSON(cJSON *pcscf_infoJSON)
{
    ...
    supi_ranges = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON, "supiRanges");
    if (supi_ranges) {
        cJSON *supi_ranges_local = NULL;
        if (!cJSON_IsArray(supi_ranges)) {
            ogs_error("OpenAPI_pcscf_info_parseFromJSON() failed [supi_ranges]");
            goto end;
        }
    ...
    gpsi_ranges = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON, "gpsiRanges");
    if (gpsi_ranges) {
        cJSON *gpsi_ranges_local = NULL;
        if (!cJSON_IsArray(gpsi_ranges)) {
            ogs_error("OpenAPI_pcscf_info_parseFromJSON() failed [gpsi_ranges]");
            goto end;
        }
    ...
    group_id = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON, "groupId");
    if (group_id) {
    if (!cJSON_IsString(group_id) && !cJSON_IsNull(group_id)) {
        ogs_error("OpenAPI_pcscf_info_parseFromJSON() failed [group_id]");
        goto end;
    }
    }

    serving_plmns = cJSON_GetObjectItemCaseSensitive(pcscf_infoJSON, "servingPlmns");
    if (serving_plmns) {
        cJSON *serving_plmns_local = NULL;
        if (!cJSON_IsArray(serving_plmns)) {
            ogs_error("OpenAPI_pcscf_info_parseFromJSON() failed [serving_plmns]");
            goto end;
        }
