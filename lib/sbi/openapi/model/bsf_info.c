...
)
{
    ...
    bsf_info_local_var->service_scenario_inds = service_scenario_inds;
    bsf_info_local_var->mac_addr_patterns = mac_addr_patterns;
...
void OpenAPI_bsf_info_free(OpenAPI_bsf_info_t *bsf_info)
{
    ...
    if (bsf_info->service_scenario_inds) {
        OpenAPI_list_free(bsf_info->service_scenario_inds);
        bsf_info->service_scenario_inds = NULL;
    }
    if (bsf_info->mac_addr_patterns) {
        OpenAPI_list_for_each(bsf_info->mac_addr_patterns, node) {
            ogs_free(node->data);
        }
        OpenAPI_list_free(bsf_info->mac_addr_patterns);
        bsf_info->mac_addr_patterns = NULL;
    }
    ogs_free(bsf_info);
}
...
cJSON *OpenAPI_bsf_info_convertToJSON(OpenAPI_bsf_info_t *bsf_info)
{
    ...
    if (bsf_info->service_scenario_inds != OpenAPI_service_scenario_ind_NULL) {
    cJSON *service_scenario_indsList = cJSON_AddArrayToObject(item, "serviceScenarioInds");
    if (service_scenario_indsList == NULL) {
        ogs_error("OpenAPI_bsf_info_convertToJSON() failed [service_scenario_inds]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->service_scenario_inds, node) {
    ...
    if (mac_addr_patternsList == NULL) {
        ogs_error("OpenAPI_bsf_info_convertToJSON() failed [mac_addr_patterns]");
        goto end;
    }
    OpenAPI_list_for_each(bsf_info->mac_addr_patterns, node) {
...
OpenAPI_bsf_info_t *OpenAPI_bsf_info_parseFromJSON(cJSON *bsf_infoJSON)
{
    ...
    service_scenario_inds = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON, "serviceScenarioInds");
    if (service_scenario_inds) {
        cJSON *service_scenario_inds_local = NULL;
        if (!cJSON_IsArray(service_scenario_inds)) {
            ogs_error("OpenAPI_bsf_info_parseFromJSON() failed [service_scenario_inds]");
            goto end;
        }
        ...
        if (service_scenario_indsList->count == 0) {
            ogs_error("OpenAPI_bsf_info_parseFromJSON() failed: Expected service_scenario_indsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }

    mac_addr_patterns = cJSON_GetObjectItemCaseSensitive(bsf_infoJSON, "macAddrPatterns");
    if (mac_addr_patterns) {
        cJSON *mac_addr_patterns_local = NULL;
        if (!cJSON_IsArray(mac_addr_patterns)) {
            ogs_error("OpenAPI_bsf_info_parseFromJSON() failed [mac_addr_patterns]");
            goto end;
        }
