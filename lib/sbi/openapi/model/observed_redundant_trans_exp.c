...
)
{
    ...
    observed_redundant_trans_exp_local_var->is_avg_e2e_pkt_delay_ul = is_avg_e2e_pkt_delay_ul;
    observed_redundant_trans_exp_local_var->avg_e2e_pkt_delay_ul = avg_e2e_pkt_delay_ul;
    observed_redundant_trans_exp_local_var->is_var_e2e_pkt_delay_ul = is_var_e2e_pkt_delay_ul;
    observed_redundant_trans_exp_local_var->var_e2e_pkt_delay_ul = var_e2e_pkt_delay_ul;
    observed_redundant_trans_exp_local_var->is_avg_e2e_pkt_delay_dl = is_avg_e2e_pkt_delay_dl;
    observed_redundant_trans_exp_local_var->avg_e2e_pkt_delay_dl = avg_e2e_pkt_delay_dl;
    observed_redundant_trans_exp_local_var->is_var_e2e_pkt_delay_dl = is_var_e2e_pkt_delay_dl;
    observed_redundant_trans_exp_local_var->var_e2e_pkt_delay_dl = var_e2e_pkt_delay_dl;
    observed_redundant_trans_exp_local_var->is_avg_e2e_pkt_loss_rate_ul = is_avg_e2e_pkt_loss_rate_ul;
    observed_redundant_trans_exp_local_var->avg_e2e_pkt_loss_rate_ul = avg_e2e_pkt_loss_rate_ul;
    observed_redundant_trans_exp_local_var->is_var_e2e_pkt_loss_rate_ul = is_var_e2e_pkt_loss_rate_ul;
    observed_redundant_trans_exp_local_var->var_e2e_pkt_loss_rate_ul = var_e2e_pkt_loss_rate_ul;
    observed_redundant_trans_exp_local_var->is_avg_e2e_pkt_loss_rate_dl = is_avg_e2e_pkt_loss_rate_dl;
    observed_redundant_trans_exp_local_var->avg_e2e_pkt_loss_rate_dl = avg_e2e_pkt_loss_rate_dl;
    observed_redundant_trans_exp_local_var->is_var_e2e_pkt_loss_rate_dl = is_var_e2e_pkt_loss_rate_dl;
    observed_redundant_trans_exp_local_var->var_e2e_pkt_loss_rate_dl = var_e2e_pkt_loss_rate_dl;
...
cJSON *OpenAPI_observed_redundant_trans_exp_convertToJSON(OpenAPI_observed_redundant_trans_exp_t *observed_redundant_trans_exp)
{
    ...
    if (cJSON_AddNumberToObject(item, "avgE2ePktDelayUl", observed_redundant_trans_exp->avg_e2e_pkt_delay_ul) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_e2e_pkt_delay_ul]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_var_e2e_pkt_delay_ul) {
    if (cJSON_AddNumberToObject(item, "varE2ePktDelayUl", observed_redundant_trans_exp->var_e2e_pkt_delay_ul) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_e2e_pkt_delay_ul]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_avg_e2e_pkt_delay_dl) {
    if (cJSON_AddNumberToObject(item, "avgE2ePktDelayDl", observed_redundant_trans_exp->avg_e2e_pkt_delay_dl) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_e2e_pkt_delay_dl]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_var_e2e_pkt_delay_dl) {
    if (cJSON_AddNumberToObject(item, "varE2ePktDelayDl", observed_redundant_trans_exp->var_e2e_pkt_delay_dl) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_e2e_pkt_delay_dl]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_avg_e2e_pkt_loss_rate_ul) {
    if (cJSON_AddNumberToObject(item, "avgE2ePktLossRateUl", observed_redundant_trans_exp->avg_e2e_pkt_loss_rate_ul) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_e2e_pkt_loss_rate_ul]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_var_e2e_pkt_loss_rate_ul) {
    if (cJSON_AddNumberToObject(item, "varE2ePktLossRateUl", observed_redundant_trans_exp->var_e2e_pkt_loss_rate_ul) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_e2e_pkt_loss_rate_ul]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_avg_e2e_pkt_loss_rate_dl) {
    if (cJSON_AddNumberToObject(item, "avgE2ePktLossRateDl", observed_redundant_trans_exp->avg_e2e_pkt_loss_rate_dl) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [avg_e2e_pkt_loss_rate_dl]");
        goto end;
    }
    }

    if (observed_redundant_trans_exp->is_var_e2e_pkt_loss_rate_dl) {
    if (cJSON_AddNumberToObject(item, "varE2ePktLossRateDl", observed_redundant_trans_exp->var_e2e_pkt_loss_rate_dl) == NULL) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_convertToJSON() failed [var_e2e_pkt_loss_rate_dl]");
        goto end;
    }
    }
...
OpenAPI_observed_redundant_trans_exp_t *OpenAPI_observed_redundant_trans_exp_parseFromJSON(cJSON *observed_redundant_trans_expJSON)
{
    ...
    avg_e2e_pkt_delay_ul = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "avgE2ePktDelayUl");
    if (avg_e2e_pkt_delay_ul) {
    if (!cJSON_IsNumber(avg_e2e_pkt_delay_ul)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_e2e_pkt_delay_ul]");
        goto end;
    }
    }

    var_e2e_pkt_delay_ul = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "varE2ePktDelayUl");
    if (var_e2e_pkt_delay_ul) {
    if (!cJSON_IsNumber(var_e2e_pkt_delay_ul)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_e2e_pkt_delay_ul]");
        goto end;
    }
    }

    avg_e2e_pkt_delay_dl = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "avgE2ePktDelayDl");
    if (avg_e2e_pkt_delay_dl) {
    if (!cJSON_IsNumber(avg_e2e_pkt_delay_dl)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_e2e_pkt_delay_dl]");
        goto end;
    }
    }

    var_e2e_pkt_delay_dl = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "varE2ePktDelayDl");
    if (var_e2e_pkt_delay_dl) {
    if (!cJSON_IsNumber(var_e2e_pkt_delay_dl)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_e2e_pkt_delay_dl]");
        goto end;
    }
    }

    avg_e2e_pkt_loss_rate_ul = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "avgE2ePktLossRateUl");
    if (avg_e2e_pkt_loss_rate_ul) {
    if (!cJSON_IsNumber(avg_e2e_pkt_loss_rate_ul)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_e2e_pkt_loss_rate_ul]");
        goto end;
    }
    }

    var_e2e_pkt_loss_rate_ul = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "varE2ePktLossRateUl");
    if (var_e2e_pkt_loss_rate_ul) {
    if (!cJSON_IsNumber(var_e2e_pkt_loss_rate_ul)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_e2e_pkt_loss_rate_ul]");
        goto end;
    }
    }

    avg_e2e_pkt_loss_rate_dl = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "avgE2ePktLossRateDl");
    if (avg_e2e_pkt_loss_rate_dl) {
    if (!cJSON_IsNumber(avg_e2e_pkt_loss_rate_dl)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [avg_e2e_pkt_loss_rate_dl]");
        goto end;
    }
    }

    var_e2e_pkt_loss_rate_dl = cJSON_GetObjectItemCaseSensitive(observed_redundant_trans_expJSON, "varE2ePktLossRateDl");
    if (var_e2e_pkt_loss_rate_dl) {
    if (!cJSON_IsNumber(var_e2e_pkt_loss_rate_dl)) {
        ogs_error("OpenAPI_observed_redundant_trans_exp_parseFromJSON() failed [var_e2e_pkt_loss_rate_dl]");
        goto end;
    }
    }
