...
void OpenAPI_problem_details_free(OpenAPI_problem_details_t *problem_details)
{
    ...
    if (problem_details->supported_api_versions) {
        OpenAPI_list_for_each(problem_details->supported_api_versions, node) {
            ogs_free(node->data);
        }
        OpenAPI_list_free(problem_details->supported_api_versions);
        problem_details->supported_api_versions = NULL;
    }
    if (problem_details->no_profile_match_info) {
        OpenAPI_no_profile_match_info_free(problem_details->no_profile_match_info);
...
cJSON *OpenAPI_problem_details_convertToJSON(OpenAPI_problem_details_t *problem_details)
{
    ...
    if (supported_api_versionsList == NULL) {
        ogs_error("OpenAPI_problem_details_convertToJSON() failed [supported_api_versions]");
        goto end;
    }
    OpenAPI_list_for_each(problem_details->supported_api_versions, node) {
    ...
    if (problem_details->no_profile_match_info) {
    cJSON *no_profile_match_info_local_JSON = OpenAPI_no_profile_match_info_convertToJSON(problem_details->no_profile_match_info);
    if (no_profile_match_info_local_JSON == NULL) {
        ogs_error("OpenAPI_problem_details_convertToJSON() failed [no_profile_match_info]");
        goto end;
    }
...
OpenAPI_problem_details_t *OpenAPI_problem_details_parseFromJSON(cJSON *problem_detailsJSON)
{
    ...
    supported_api_versions = cJSON_GetObjectItemCaseSensitive(problem_detailsJSON, "supportedApiVersions");
    if (supported_api_versions) {
        cJSON *supported_api_versions_local = NULL;
        if (!cJSON_IsArray(supported_api_versions)) {
            ogs_error("OpenAPI_problem_details_parseFromJSON() failed [supported_api_versions]");
            goto end;
        }
    ...
    no_profile_match_info = cJSON_GetObjectItemCaseSensitive(problem_detailsJSON, "noProfileMatchInfo");
