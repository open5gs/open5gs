...
cJSON *OpenAPI_immediate_report_convertToJSON(OpenAPI_immediate_report_t *immediate_report)
{
    ...
    if ((immediate_report->subscription_data_sets != NULL) +
            (immediate_report->shared_data_list != NULL) != 1) {
        ogs_error("OpenAPI_immediate_report_convertToJSON() failed [ImmediateReport]: exactly one alternative must be set");
        return NULL;
    }

    if (immediate_report->subscription_data_sets) {
        return OpenAPI_subscription_data_sets_convertToJSON(immediate_report->subscription_data_sets);
    }
    if (immediate_report->shared_data_list) {
...
{
    OpenAPI_lnode_t *node = NULL;
    OpenAPI_subscription_data_sets_t *subscription_data_sets = NULL;
    OpenAPI_list_t *shared_data_list = NULL;

    if (cJSON_IsObject(immediate_reportJSON)) {
        subscription_data_sets = OpenAPI_subscription_data_sets_parseFromJSON(immediate_reportJSON);
        if (!subscription_data_sets) {
            ogs_error("OpenAPI_subscription_data_sets_parseFromJSON failed [subscription_data_sets]");
            goto end;
        }
    } else if (cJSON_IsArray(immediate_reportJSON)) {
        cJSON *shared_data_list_local = NULL;

        shared_data_list = OpenAPI_list_create();

        cJSON_ArrayForEach(shared_data_list_local, immediate_reportJSON) {
        ...
        ogs_error("OpenAPI_immediate_report_parseFromJSON() failed [ImmediateReport]");
        goto end;
    }

    return OpenAPI_immediate_report_create(subscription_data_sets, shared_data_list);
end:
    if (subscription_data_sets) {
        OpenAPI_subscription_data_sets_free(subscription_data_sets);
