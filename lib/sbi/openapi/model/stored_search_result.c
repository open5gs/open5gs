...
)
{
    ...
    stored_search_result_local_var->complete_nf_instances = complete_nf_instances;
...
void OpenAPI_stored_search_result_free(OpenAPI_stored_search_result_t *stored_search_result)
{
    ...
    if (stored_search_result->complete_nf_instances) {
        OpenAPI_list_for_each(stored_search_result->complete_nf_instances, node) {
            OpenAPI_nf_profile_free(node->data);
        }
        OpenAPI_list_free(stored_search_result->complete_nf_instances);
        stored_search_result->complete_nf_instances = NULL;
    }
...
cJSON *OpenAPI_stored_search_result_convertToJSON(OpenAPI_stored_search_result_t *stored_search_result)
{
    ...
    if (complete_nf_instancesList == NULL) {
        ogs_error("OpenAPI_stored_search_result_convertToJSON() failed [complete_nf_instances]");
        goto end;
    }
    OpenAPI_list_for_each(stored_search_result->complete_nf_instances, node) {
        cJSON *itemLocal = OpenAPI_nf_profile_convertToJSON(node->data);
...
OpenAPI_stored_search_result_t *OpenAPI_stored_search_result_parseFromJSON(cJSON *stored_search_resultJSON)
{
    ...
    complete_nf_instances = cJSON_GetObjectItemCaseSensitive(stored_search_resultJSON, "completeNfInstances");
    if (complete_nf_instances) {
        cJSON *complete_nf_instances_local = NULL;
        if (!cJSON_IsArray(complete_nf_instances)) {
            ogs_error("OpenAPI_stored_search_result_parseFromJSON() failed [complete_nf_instances]");
            goto end;
        }
