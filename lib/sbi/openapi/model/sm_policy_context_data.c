...
)
{
    ...
    sm_policy_context_data_local_var->alt_slice_info = alt_slice_info;
    ...
    sm_policy_context_data_local_var->atsss_capabs = atsss_capabs;
    ...
    sm_policy_context_data_local_var->ursp_enf_info = ursp_enf_info;
    sm_policy_context_data_local_var->ssc_mode = ssc_mode;
    sm_policy_context_data_local_var->ue_req_dnn = ue_req_dnn;
    sm_policy_context_data_local_var->ue_req_pdu_session_type = ue_req_pdu_session_type;
    sm_policy_context_data_local_var->is_hrsbo_ind = is_hrsbo_ind;
    sm_policy_context_data_local_var->hrsbo_ind = hrsbo_ind;
    sm_policy_context_data_local_var->is_loc_offload_ind = is_loc_offload_ind;
    sm_policy_context_data_local_var->loc_offload_ind = loc_offload_ind;
    sm_policy_context_data_local_var->ue_pol_fail_report = ue_pol_fail_report;
    sm_policy_context_data_local_var->is_ursp_prov_supp_ind = is_ursp_prov_supp_ind;
    sm_policy_context_data_local_var->ursp_prov_supp_ind = ursp_prov_supp_ind;
    sm_policy_context_data_local_var->is_mpx_media_ind = is_mpx_media_ind;
    sm_policy_context_data_local_var->mpx_media_ind = mpx_media_ind;
    sm_policy_context_data_local_var->traffic_descriptors = traffic_descriptors;
...
void OpenAPI_sm_policy_context_data_free(OpenAPI_sm_policy_context_data_t *sm_policy_context_data)
{
    ...
    if (sm_policy_context_data->alt_slice_info) {
        OpenAPI_snssai_free(sm_policy_context_data->alt_slice_info);
        sm_policy_context_data->alt_slice_info = NULL;
    }
    ...
    if (sm_policy_context_data->atsss_capabs) {
        OpenAPI_list_free(sm_policy_context_data->atsss_capabs);
        sm_policy_context_data->atsss_capabs = NULL;
    }
    ...
    if (sm_policy_context_data->ursp_enf_info) {
        ogs_free(sm_policy_context_data->ursp_enf_info);
        sm_policy_context_data->ursp_enf_info = NULL;
    }
    if (sm_policy_context_data->ue_req_dnn) {
        ogs_free(sm_policy_context_data->ue_req_dnn);
        sm_policy_context_data->ue_req_dnn = NULL;
    }
    if (sm_policy_context_data->traffic_descriptors) {
        OpenAPI_list_for_each(sm_policy_context_data->traffic_descriptors, node) {
            OpenAPI_ddd_traffic_descriptor_free(node->data);
        }
        OpenAPI_list_free(sm_policy_context_data->traffic_descriptors);
        sm_policy_context_data->traffic_descriptors = NULL;
    }
...
cJSON *OpenAPI_sm_policy_context_data_convertToJSON(OpenAPI_sm_policy_context_data_t *sm_policy_context_data)
{
    ...
    if (sm_policy_context_data->alt_slice_info) {
    cJSON *alt_slice_info_local_JSON = OpenAPI_snssai_convertToJSON(sm_policy_context_data->alt_slice_info);
    if (alt_slice_info_local_JSON == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [alt_slice_info]");
        goto end;
    }
    ...
    if (sm_policy_context_data->atsss_capabs != OpenAPI_atsss_capability_ext_NULL) {
    cJSON *atsss_capabsList = cJSON_AddArrayToObject(item, "atsssCapabs");
    if (atsss_capabsList == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [atsss_capabs]");
        goto end;
    }
    OpenAPI_list_for_each(sm_policy_context_data->atsss_capabs, node) {
    ...
    if (sm_policy_context_data->ursp_enf_info) {
    if (cJSON_AddStringToObject(item, "urspEnfInfo", sm_policy_context_data->ursp_enf_info) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [ursp_enf_info]");
        goto end;
    }
    }

    if (sm_policy_context_data->ssc_mode != OpenAPI_ssc_mode_NULL) {
    if (cJSON_AddStringToObject(item, "sscMode", OpenAPI_ssc_mode_ToString(sm_policy_context_data->ssc_mode)) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [ssc_mode]");
        goto end;
    }
    }

    if (sm_policy_context_data->ue_req_dnn) {
    if (cJSON_AddStringToObject(item, "ueReqDnn", sm_policy_context_data->ue_req_dnn) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [ue_req_dnn]");
        goto end;
    }
    }

    if (sm_policy_context_data->ue_req_pdu_session_type != OpenAPI_pdu_session_type_NULL) {
    if (cJSON_AddStringToObject(item, "ueReqPduSessionType", OpenAPI_pdu_session_type_ToString(sm_policy_context_data->ue_req_pdu_session_type)) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [ue_req_pdu_session_type]");
        goto end;
    }
    }

    if (sm_policy_context_data->is_hrsbo_ind) {
    if (cJSON_AddBoolToObject(item, "hrsboInd", sm_policy_context_data->hrsbo_ind) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [hrsbo_ind]");
        goto end;
    }
    }

    if (sm_policy_context_data->is_loc_offload_ind) {
    if (cJSON_AddBoolToObject(item, "locOffloadInd", sm_policy_context_data->loc_offload_ind) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [loc_offload_ind]");
        goto end;
    }
    }

    if (sm_policy_context_data->ue_pol_fail_report != OpenAPI_n1_n2_message_transfer_cause_NULL) {
    if (cJSON_AddStringToObject(item, "uePolFailReport", OpenAPI_n1_n2_message_transfer_cause_ToString(sm_policy_context_data->ue_pol_fail_report)) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [ue_pol_fail_report]");
        goto end;
    }
    }

    if (sm_policy_context_data->is_ursp_prov_supp_ind) {
    if (cJSON_AddBoolToObject(item, "urspProvSuppInd", sm_policy_context_data->ursp_prov_supp_ind) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [ursp_prov_supp_ind]");
        goto end;
    }
    }

    if (sm_policy_context_data->is_mpx_media_ind) {
    if (cJSON_AddBoolToObject(item, "mpxMediaInd", sm_policy_context_data->mpx_media_ind) == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [mpx_media_ind]");
        goto end;
    }
    }
    ...
    if (traffic_descriptorsList == NULL) {
        ogs_error("OpenAPI_sm_policy_context_data_convertToJSON() failed [traffic_descriptors]");
        goto end;
    }
    OpenAPI_list_for_each(sm_policy_context_data->traffic_descriptors, node) {
...
OpenAPI_sm_policy_context_data_t *OpenAPI_sm_policy_context_data_parseFromJSON(cJSON *sm_policy_context_dataJSON)
{
    ...
    alt_slice_info = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "altSliceInfo");
    if (alt_slice_info) {
    ...
    atsss_capabs = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "atsssCapabs");
    if (atsss_capabs) {
        cJSON *atsss_capabs_local = NULL;
        if (!cJSON_IsArray(atsss_capabs)) {
            ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [atsss_capabs]");
            goto end;
        }
        ...
        if (atsss_capabsList->count == 0) {
            ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed: Expected atsss_capabsList to not be empty (after ignoring unsupported enum values).");
            goto end;
        }
    }
    ...
    ursp_enf_info = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "urspEnfInfo");
    if (ursp_enf_info) {
    if (!cJSON_IsString(ursp_enf_info) && !cJSON_IsNull(ursp_enf_info)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [ursp_enf_info]");
        goto end;
    }
    }

    ssc_mode = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "sscMode");
    if (ssc_mode) {
    if (!cJSON_IsString(ssc_mode)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [ssc_mode]");
        goto end;
    }
    ssc_modeVariable = OpenAPI_ssc_mode_FromString(ssc_mode->valuestring);
    }

    ue_req_dnn = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "ueReqDnn");
    if (ue_req_dnn) {
    if (!cJSON_IsString(ue_req_dnn) && !cJSON_IsNull(ue_req_dnn)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [ue_req_dnn]");
        goto end;
    }
    }

    ue_req_pdu_session_type = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "ueReqPduSessionType");
    if (ue_req_pdu_session_type) {
    if (!cJSON_IsString(ue_req_pdu_session_type)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [ue_req_pdu_session_type]");
        goto end;
    }
    ue_req_pdu_session_typeVariable = OpenAPI_pdu_session_type_FromString(ue_req_pdu_session_type->valuestring);
    }

    hrsbo_ind = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "hrsboInd");
    if (hrsbo_ind) {
    if (!cJSON_IsBool(hrsbo_ind)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [hrsbo_ind]");
        goto end;
    }
    }

    loc_offload_ind = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "locOffloadInd");
    if (loc_offload_ind) {
    if (!cJSON_IsBool(loc_offload_ind)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [loc_offload_ind]");
        goto end;
    }
    }

    ue_pol_fail_report = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "uePolFailReport");
    if (ue_pol_fail_report) {
    if (!cJSON_IsString(ue_pol_fail_report)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [ue_pol_fail_report]");
        goto end;
    }
    ue_pol_fail_reportVariable = OpenAPI_n1_n2_message_transfer_cause_FromString(ue_pol_fail_report->valuestring);
    }

    ursp_prov_supp_ind = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "urspProvSuppInd");
    if (ursp_prov_supp_ind) {
    if (!cJSON_IsBool(ursp_prov_supp_ind)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [ursp_prov_supp_ind]");
        goto end;
    }
    }

    mpx_media_ind = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "mpxMediaInd");
    if (mpx_media_ind) {
    if (!cJSON_IsBool(mpx_media_ind)) {
        ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [mpx_media_ind]");
        goto end;
    }
    }

    traffic_descriptors = cJSON_GetObjectItemCaseSensitive(sm_policy_context_dataJSON, "trafficDescriptors");
    if (traffic_descriptors) {
        cJSON *traffic_descriptors_local = NULL;
        if (!cJSON_IsArray(traffic_descriptors)) {
            ogs_error("OpenAPI_sm_policy_context_data_parseFromJSON() failed [traffic_descriptors]");
            goto end;
        }
    ...
    sm_policy_context_data_local_var = OpenAPI_sm_policy_context_data_create (
        ...
        loc_offload_ind ? true : false,
        loc_offload_ind ? loc_offload_ind->valueint : 0,
        ue_pol_fail_report ? ue_pol_fail_reportVariable : 0,
        ursp_prov_supp_ind ? true : false,
        ursp_prov_supp_ind ? ursp_prov_supp_ind->valueint : 0,
        mpx_media_ind ? true : false,
        mpx_media_ind ? mpx_media_ind->valueint : 0,
