...
{
    OpenAPI_mrf_info_t *mrf_info_local_var = ogs_malloc(sizeof(OpenAPI_mrf_info_t));
    ogs_assert(mrf_info_local_var);

    mrf_info_local_var->media_capability_list = media_capability_list;

    return mrf_info_local_var;
}
...
void OpenAPI_mrf_info_free(OpenAPI_mrf_info_t *mrf_info)
{
    ...
    if (mrf_info->media_capability_list) {
        OpenAPI_list_for_each(mrf_info->media_capability_list, node) {
            ogs_free(node->data);
        }
        OpenAPI_list_free(mrf_info->media_capability_list);
        mrf_info->media_capability_list = NULL;
    }
    ogs_free(mrf_info);
}
...
cJSON *OpenAPI_mrf_info_convertToJSON(OpenAPI_mrf_info_t *mrf_info)
{
    ...
    if (mrf_info == NULL) {
        ogs_error("OpenAPI_mrf_info_convertToJSON() failed [MrfInfo]");
        return NULL;
    }
    ...
    if (media_capability_listList == NULL) {
        ogs_error("OpenAPI_mrf_info_convertToJSON() failed [media_capability_list]");
        goto end;
    }
    OpenAPI_list_for_each(mrf_info->media_capability_list, node) {
...
{
    OpenAPI_mrf_info_t *mrf_info_local_var = NULL;
    OpenAPI_lnode_t *node = NULL;
    cJSON *media_capability_list = NULL;
    OpenAPI_list_t *media_capability_listList = NULL;
    media_capability_list = cJSON_GetObjectItemCaseSensitive(mrf_infoJSON, "mediaCapabilityList");
    if (media_capability_list) {
        cJSON *media_capability_list_local = NULL;
        if (!cJSON_IsArray(media_capability_list)) {
            ogs_error("OpenAPI_mrf_info_parseFromJSON() failed [media_capability_list]");
            goto end;
        }
    ...
    mrf_info_local_var = OpenAPI_mrf_info_create (
...
OpenAPI_mrf_info_t *OpenAPI_mrf_info_copy(OpenAPI_mrf_info_t *dst, OpenAPI_mrf_info_t *src)
{
    ...
    ogs_assert(src);
    item = OpenAPI_mrf_info_convertToJSON(src);
    if (!item) {
        ogs_error("OpenAPI_mrf_info_convertToJSON() failed");
        return NULL;
    }
    ...
    OpenAPI_mrf_info_free(dst);
    dst = OpenAPI_mrf_info_parseFromJSON(item);
    cJSON_Delete(item);
